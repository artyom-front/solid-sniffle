...
src/app/api/        public/* — данные для сайта; admin/* — CRUD, auth, TOTP, аудит, настройки
src/lib/engine/     таблицы, расписание, дисциплина, LIVE-сигналы
src/lib/services/   выборки для страниц
prisma/             схема БД (User, Banner, Setting…), seed, bootstrap админа
...
- Первый вход — `ADMIN_EMAIL` / `ADMIN_PASSWORD` из `.env` (создаётся
  bootstrap'ом при деплое, если пользователей ещё нет).
- **Пользователи** (раздел в админке, только супер-админ): выдача
  доступов другим ролям, сброс пароля, отключение 2FA сотруднику
  (утерян телефон), удаление аккаунтов.
- **Логин по TOTP** — глобальный тумблер там же: включён — при входе
  запрашивается код из приложения; выключен (аварийно) — вход только
  по паролю. Хранится в таблице `Setting`, все переключения в аудите.
