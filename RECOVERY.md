...
| Файл | Что это |
|---|---|
| `scoresbox-onefile.git-bundle` (~0,5 МБ) | ✅ **ГЛАВНЫЙ**: весь проект одним коммитом, применяется в ЛЮБОМ репозитории (даже после reinit). Если была ошибка «prerequisite commits» — использовать его |
| `scoresbox-update.git-bundle` (~230 КБ) | добавка поверх старой истории 30e60c2 (только если репо НЕ пересоздавалось) |
...
git fetch "scoresbox-onefile.git-bundle" main
