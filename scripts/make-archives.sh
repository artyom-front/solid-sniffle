...
ONEFILE=download/scoresbox-2026-09-07-execfix.git-bundle
rm -f "$ONEFILE"
...
( cd "$SQUASH" \
  ...
  && git commit -q -m "SCORESBOX 2026-09-07 execfix: CD вызывает deploy/rollback через bash — фикс exit 126 «Permission denied» (бит x теряется при заливке файлов на VPS с Windows) + все прежние фиксы (GHCR packages:write, Docker USER 1001, actions v7) — актуальная поставка solid-sniffle (клиент, админка, деплой)" \
  && git branch -M main \
  && git bundle create "$ROOT/$ONEFILE" main ) >/dev/null
cp "$ONEFILE" public/scoresbox-2026-09-07-execfix.git-bundle
...
echo "    public/download/scoresbox-2026-09-07-execfix.git-bundle — для ссылки /download/"
...
| `scoresbox-2026-09-07-execfix.git-bundle` | ~0,5 МБ | ✅ **ГЛАВНЫЙ ФАЙЛ (актуален на 07.09.2026, фикс exit 126)**: весь проект одним коммитом + все фиксы (exec, GHCR-права, Docker), применяется в ЛЮБОМ репозитории | **основной путь** (работает даже после reinit) |
...
git log --oneline -1 FETCH_HEAD       # проверка: «SCORESBOX 2026-09-07 execfix», НЕ «ghcr» / «2026-09-04»
...
Если имя скачанного файла не содержит `2026-09-07-execfix` — файл старый.
Если после `git log --oneline -1 FETCH_HEAD` видно сообщение без слова `execfix`
(только «SCORESBOX 2026-09-07: GITHUB_TOKEN…», «2026-09-04», v1.0.4 и т.п.) — файл
старый: в актуальном сообщении первой строкой дата 2026-09-07 и слово execfix.
