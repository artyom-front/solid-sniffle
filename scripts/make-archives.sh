...
ONEFILE=download/scoresbox-2026-09-04-dockerfix.git-bundle
rm -f "$ONEFILE"
...
( cd "$SQUASH" \
  ...
  && git commit -q -m "SCORESBOX 2026-09-04: fix Docker exit 127 (USER 1001:1001) + cd.yml rollback + actions v7 — актуальная поставка solid-sniffle (клиент, админка, деплой)" \
  && git branch -M main \
  && git bundle create "$ROOT/$ONEFILE" main ) >/dev/null
cp "$ONEFILE" public/scoresbox-2026-09-04-dockerfix.git-bundle
