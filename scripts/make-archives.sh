...
ONEFILE=download/scoresbox-2026-09-08-admin2.git-bundle
rm -f "$ONEFILE"
...
( cd "$SQUASH" \
  ...
  && git commit -q -m "SCORESBOX 2026-09-08 admin2: массовый импорт CSV (заявки/персоны/команды/стадионы/клубы, идемпотентно) + персоны с ролями (судья/VAR/тренер/врач/… + кастомные) и фото + медиатека (фото клубов/команд/стадионов/баннеров, файлы протоколов PDF/скан) + товарищеские матчи (без лиг и сезонов) + списки обновляются без F5 + антидубли + панель «Пользователи и доступы» (роли, сброс 2FA, блокировка) + админ-визард чемпионата + загрузка протокола + favicon + футер + фильтры главной Live/Завершённые и даты ±10 дней + баннеры с картинкой — актуальная поставка solid-sniffle (клиент, админка, деплой)" \
  && git branch -M main \
  && git bundle create "$ROOT/$ONEFILE" main ) >/dev/null
cp "$ONEFILE" public/scoresbox-2026-09-08-admin2.git-bundle
...
# 📦 Поставка SCORESBOX (обновление «admin2» от 08.09.2026)

**Что это:** версия с массовыми операциями для админки. Заявку лиги из 10 команд
× 50 игроков теперь можно завести за минуты вместо дней.

## Что нового

- **Импорт CSV** (Справочники → Импорт): заявки игроков/тренеров, персоны, команды,
  стадионы, клубы — вставкой из Excel или файлом; «Проверить» без записи; повторы не дублируются
- **Персоны с ролями**: ФИО/пол/фото отдельно от специализаций (игрок, судья, VAR,
  тренер, врач, делегат, президент… + свои роли); защита от дублей по ФИО и дате рождения
- **Фото и файлы**: эмблемы клубов/команд, фото персон и стадионов, картинки баннеров,
  скан/PDF бумажного протокола (вкладка «Файл протокола»)
- **Товарищеские матчи**: раздел «Матчи» → вкладка «Товарищеские» — без лиги/сезона,
  не попадают в таблицы и дисциплину
- **Списки обновляются без F5** после создания/удаления
- **Панель «Пользователи и доступы»** (Супер-админ): создание учёток с ролями,
  сброс пароля, отключение входа, сброс 2FA
- **Понятная цепочка чемпионата**: подсказки «лига → сезон → заявки → расписание»,
  генератор календаря (раздел «Расписание») — существовал и раньше, теперь виднее
- Публичный сайт: favicon, чистый футер, фильтры «Live»/«Завершённые» + даты ± 10 дней

## Скачать

**Прямая ссылка (надёжная):**

https://preview-chat-d2608ef6-93f5-4d0d-bfb4-6435f0304186.space-z.ai/download/scoresbox-2026-09-08-admin2.git-bundle
...
| Файл | Размер | Что это |
|---|---|---|
| `scoresbox-2026-09-08-admin2.git-bundle` | ~0,7 МБ | ✅ **ГЛАВНЫЙ ФАЙЛ**: весь проект одним коммитом, работает в ЛЮБОМ репозитории |
...
git fetch "$HOME\Downloads\scoresbox-2026-09-08-admin2.git-bundle" main
git log --oneline -1 FETCH_HEAD       # проверка: «SCORESBOX 2026-09-08 admin2...»
git reset --hard FETCH_HEAD && git push --force origin main
```

## Проверка актуальности

- В имени файла: `2026-09-08-admin2`. Есть дата старее или другое слово
  (cleanup/composefix/execfix/…) — файл старый, скачайте по ссылке выше.
- В сообщении коммита (`git log --oneline -1 FETCH_HEAD`): первая строка
  начинается с `SCORESBOX 2026-09-08 admin2:`.

После push: новый тег **v1.0.14** задеплоит изменения (миграция БД — `prisma db push`
выполнится деплоем автоматически, данные не теряются). Подробности — DEPLOY.md §I.7.
EOF
...
echo "    public/download/scoresbox-2026-09-08-admin2.git-bundle — для ссылки /download/"
