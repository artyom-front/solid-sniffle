...
#   sudo ./scripts/setup-nginx.sh scoresbox.ru       # текущий домен
#   sudo ./scripts/setup-nginx.sh <новый-домен>     # при переезде
#
# Что делает:
...
DOMAIN="${1:?Использование: ./scripts/setup-nginx.sh <домен, например scoresbox.ru>}"

# валидация: похоже ли на домен (scoresbox.ru, www тоже ок)
if ! [[ "$DOMAIN" =~ ^([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)+[a-z]{2,}$ ]]; then
  echo "!! «$DOMAIN» не похоже на домен (ожидается вида scoresbox.ru)"
  exit 1
fi
...
server {
    ...
    # SECURITY/UPLOAD: backend принимает изображения до 6 МБ и PDF до 10 МБ
    # (src/app/api/admin/media). 2m обрезал загрузку на уровне nginx (413)
    # раньше, чем файл доходил до Next.js. 12m = лимит + запас на multipart-обёртку.
