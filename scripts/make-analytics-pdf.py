...
def _draw_footer(c, doc, num):
    ...
    c.drawString(doc.leftMargin, PAGE_H - 0.52 * inch, 'SCORESBOX · Футбол Чувашии · scoresbox.ru')
...
story.append(P('<b>Вывод третий.</b> Домен — параметр, а не константа: SITE_URL в .env, '
               ...
               'Переезд на новый домен — штатная операция на 15 минут (см. DEPLOY.md).'))
...
story += h2_block('8.4 Где живёт домен (для будущей смены)', [P(
    'Домен вынесен из кода в параметры. При переезде (например, со старого домена → '
