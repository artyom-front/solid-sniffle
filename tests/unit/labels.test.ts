...
describe("Словари UI · инварианты нейминга", () => {
  ...
  test("события протокола: полный набор + VAR + замены", () => {
    for (const e of [
      ...
      "SUBSTITUTION", "SUB_OUT", "SUB_IN", "VAR_GOAL_CONFIRM", "VAR_GOAL_CANCEL", "VAR_PENALTY",
    ]) {
      expect(EVENT_LABELS[e]).toBeTruthy();
    }
    // короткие подписи — только там, где иконка не исчерпывающа
    // (пенальти — иконка с бейджем «П», подпись не нужна)
    expect(EVENT_SHORT_LABELS.OWN_GOAL).toBeTruthy();
    expect(EVENT_SHORT_LABELS.GOAL).toBeUndefined();
    expect(EVENT_SHORT_LABELS.PENALTY).toBeUndefined();
    expect(EVENT_SHORT_LABELS.YELLOW_CARD).toBeUndefined();
    expect(EVENT_SHORT_LABELS.SUBSTITUTION).toBeUndefined();
  });
