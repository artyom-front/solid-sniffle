...
import { normalizeRoles, refereeFromRoles, cardRoleConflict, CARD_ROLE_CONFLICT_HINT, PLAYER_CONFLICT_CODES, roleName } from "@/lib/roles";
...
export async function POST(req: Request) {
  try {
    ...
    // Предохранитель: U+FFFD = кириллица погибла при чтении файла не в UTF-8
    // (клиент обычно распознаёт кодировку сам, но вставка текста — без гарантий)
    if (csv.includes("\uFFFD")) {
      throw new HttpError(
        422,
        "Текст повреждён: кириллица не прочиталась (файл не в UTF-8). " +
          "Откройте таблицу в Excel, скопируйте её (Ctrl+C) и вставьте в поле импорта (Ctrl+V), " +
          "либо пересохраните файл как «CSV UTF-8»."
      );
    }
    ...
    // SECURITY (аудит Task 27 🟠-1, v1.0.24): импорт не должен давать
    // CLUB_ADMIN больше прав, чем одиночные роуты. В registrations/teams
    // POST клубный скоуп форсируется (v1.0.21), а импорт его обходил:
    // заявки заводились в ЧУЖИЕ команды, команды — в любые клубы.
    const isClubAdmin = user.role === "CLUB_ADMIN";
    if (isClubAdmin && entity === "clubs") {
      throw new HttpError(403, "Импорт клубов доступен только администратору лиги");
    }
    if (isClubAdmin && !user.clubId) {
      throw new HttpError(403, "У клубного администратора не указан клуб — обратитесь к администратору лиги");
    }
    const forceClubId = isClubAdmin ? user.clubId : null;

    let season: { id: string; startDate: Date } | null = null;
    let team: { id: string; name: string; clubId: string | null } | null = null;
    if (entity === "players") {
      if (!teamId || !seasonId) throw new HttpError(422, "Для импорта заявки укажите команду и сезон");
      team = await db.team.findUnique({ where: { id: teamId }, select: { id: true, name: true, clubId: true } });
      ...
      // BOLA-фикс: заявки — только в команды своего клуба (как в registrations POST)
      if (isClubAdmin && team.clubId !== user.clubId) {
        throw new HttpError(403, "Клубный администратор может заводить заявки только в команды своего клуба");
      }
    }
...
async function importRow(
  ...
  ctx: { teamId: string | null; seasonId: string | null; seasonStart: Date; closePrevious: boolean; forceClubId: string | null }
  ...
  if (entity === "teams") {
    ...
    if (ctx.forceClubId) {
      // BOLA-фикс (v1.0.24): CLUB_ADMIN создаёт команды ТОЛЬКО в своём
      // клубе (как в teams POST) — клуб из строки игнорируется/проверяется
      clubId = ctx.forceClubId;
      if (clubName) {
        const named = await tx.club.findFirst({ where: { name: { equals: clubName, mode: "insensitive" } } });
        if (named && named.id !== ctx.forceClubId) {
          throw new HttpError(403, `Клубный администратор может заводить команды только в своём клубе (в строке указан клуб «${clubName}»)`);
        }
      }
    } else if (clubName) {
  ...
  // «Судья ≠ игрок»: в одной строке/ячейке роли несовместимы
  const rowConflict = cardRoleConflict(roles);
  if (rowConflict.length > 0) {
    throw new HttpError(
      422,
      `Роль «Игрок» несовместима с ${rowConflict.map(roleName).join(", ")} в одной строке. ` + CARD_ROLE_CONFLICT_HINT
    );
  }

  const norm = (s: string) => s.trim().toLowerCase().replace(/\s+/g, " ");
  // ключ дедупликации: ФИО (без учёта регистра) + дата рождения, если указана.
  // ВАЖНО: ищем и по исходному написанию, и по нижнему регистру — в базах
  // с коллацией C (контейнеры без ru_LOCALE) LOWER() не складывает кириллицу
  // и mode:insensitive молча перестаёт матчить; дубль по написанию ловим точно.
  let sameFio = await tx.person.findMany({
    where: {
      AND: [
        {
          OR: [
        ...
        ...(middleName ? [{ OR: [
          { middleName: { equals: middleName.trim(), mode: "insensitive" as const } },
          { middleName: { equals: norm(middleName), mode: "insensitive" as const } },
        ] }] : []),
        ...(birthDate ? [{ birthDate }] : []),
      ],
    },
  ...
  // Однофамильцы (фидбек v1.0.20): строка БЕЗ даты рождения и отчества не
  // различает тёзок. Если найденный человек уже активно заявлен за ДРУГУЮ
  // команду этого сезона — считаем его другим человеком и создаём отдельный
  // профиль (позже профили можно смёржить). Повторный импорт в ту же команду
  // по-прежнему идемпотентен: заявка в этой команде не считается конфликтом.
  let namesakeSplit = false;
  if (entity === "players" && !birthDate && !middleName && sameFio.length > 0 && ctx.seasonId && ctx.teamId) {
    const busyElsewhere = await tx.registration.findMany({
      where: {
        personId: { in: sameFio.map((p) => p.id) },
        seasonId: ctx.seasonId,
        status: "ACTIVE",
        endDate: null,
        teamId: { not: ctx.teamId },
      },
      select: { personId: true },
    });
    const busy = new Set(busyElsewhere.map((r) => r.personId));
    const free = sameFio.filter((p) => !busy.has(p.id));
    if (free.length === 0 && busy.size > 0) {
      namesakeSplit = true;
      sameFio = [];
    } else {
      sameFio = free;
  ...
  if (person) {
    ...
    // Заявка (players): роль «Игрок» НЕ пишется на карточку судьи —
    ...
    // играет в лиге A). Сезонная проверка заявки — ниже, перед create.
    const incomingRoles =
      entity === "players" && person.roles.some((c) => PLAYER_CONFLICT_CODES.includes(c))
        ? roles.filter((c) => c !== "PLAYER")
        : roles;
    const mergedRoles = [...new Set([...person.roles, ...incomingRoles])];
    // «Судья ≠ игрок» на карточке: мердж ролей не должен создать конфликт
    // (карточка — игрок, а строка добавляет судейскую роль)
    const mergeConflict = cardRoleConflict(mergedRoles);
    if (mergeConflict.length > 0) {
      throw new HttpError(
        409,
        `Карточка уже с ролью игрока — строка добавляет несовместимую судейскую роль (${mergeConflict.map(roleName).join(", ")}). ` + CARD_ROLE_CONFLICT_HINT
      );
    }
  ...
  // «Судья ≠ игрок» в ЭТОМ сезоне: заявка игрока запрещена, если человек
  // уже назначен судьёй на матчи сезона (в другой лиге — можно)
  if (regRole === "PLAYER" && ctx.seasonId) {
    await assertPlayerRegistrationAllowed(tx, person.id, ctx.seasonId);
  }

  // v1.0.23: история заявок разрешена — дедупликация импорта по АКТИВНОЙ заявке
  const existingReg = await tx.registration.findFirst({
    where: { personId: person.id, teamId: ctx.teamId!, seasonId: ctx.seasonId!, endDate: null },
  });
  if (existingReg) {
  ...
  const notes = [
    ...(namesakeSplit ? ["однофамилец — создан отдельный профиль (потом можно смёржить)"] : []),
    ...(transferNote ? [transferNote] : []),
  ];
  return { status: "created", name: fullName, message: notes.length > 0 ? notes.join("; ") : undefined };
}
