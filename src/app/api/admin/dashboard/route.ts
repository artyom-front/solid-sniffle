...
export async function GET() {
  try {
    ...
    // v1.0.32: админ конкретной лиги видит только её матчи/дисквалы —
    // дашборд утилитарный: всё, что он показывает, можно ПОКЛИКАТЬ и открыть
    ...
    const matchWhere = leagueScope ? { stage: { season: { leagueId: leagueScope } } } : {};
    const [matches, activeSuspensions, recentAudit] = await Promise.all([
      ...
      db.suspension.findMany({
        where: leagueScope ? { isActive: true, season: { leagueId: leagueScope } } : { isActive: true },
        select: { id: true },
      }),
