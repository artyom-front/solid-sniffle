...
export async function PATCH(req: Request) {
  try {
    ...
    switch (action) {
      case "resetPassword": {
        if (!password || String(password).length < 8) throw new HttpError(422, "Пароль — минимум 8 символов");
        // sessionVersion++ — мгновенно убивает все текущие сессии пользователя
        await db.user.update({
          where: { id },
          data: { passwordHash: hashPassword(String(password)), sessionVersion: { increment: 1 } },
        });
      ...
      case "resetTotp": {
        // sessionVersion++ — сброс 2FA тоже invalideирует активные сессии
