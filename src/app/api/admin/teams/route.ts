...
export async function POST(req: Request) {
  try {
    ...
    // SECURITY (BOLA): CLUB_ADMIN создаёт команды только в своём клубе —
    // присланный clubId другого клуба игнорируется, принадлежность форсится.
    let targetClubId = clubId || null;
    if (user.role === "CLUB_ADMIN") {
      if (!user.clubId) throw new HttpError(403, "У клубного администратора не указан клуб — обратитесь к администратору лиги");
      targetClubId = user.clubId;
    }
    if (targetClubId) {
      const club = await db.club.findUnique({ where: { id: targetClubId } });
      if (!club) throw new HttpError(404, "Клуб не найден");
    }
    const team = await db.team.create({
      data: { name: String(name).trim(), clubId: targetClubId, city: city || null, logoUrl: logoUrl || null },
    });
