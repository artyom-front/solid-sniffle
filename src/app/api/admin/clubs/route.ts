...
export async function GET() {
  try {
    ...
    return Response.json({
      clubs: clubs.map((c) => ({
        id: c.id, name: c.name, city: c.city, description: c.description, logoUrl: c.logoUrl,
...
export async function POST(req: Request) {
  try {
    // SECURITY: создание клубов — только уровень лиги. CLUB_ADMIN управляет
    // своим клубом, но не может плодить новые (иначе роль выходит за скоуп).
    const user = await requireRole("LEAGUE_ADMIN", "SUPER_ADMIN");
    const { name, city, description, logoUrl } = await req.json();
    if (!name || String(name).trim() === "") throw new HttpError(422, "Укажите название клуба");
    const club = await db.club.create({
      data: {
        name: String(name).trim(), city: city || null, description: description || null,
        logoUrl: typeof logoUrl === "string" && logoUrl.startsWith("/api/media/") ? logoUrl : null,
      },
    });
