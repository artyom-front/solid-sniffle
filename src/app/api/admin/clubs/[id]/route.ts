...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    // SECURITY (BOLA): клубный администратор правит ТОЛЬКО свой клуб —
    // иначе id любого клуба из БД открывал бы чужие данные для PATCH.
    if (user.role === "CLUB_ADMIN" && id !== user.clubId) {
      throw new HttpError(403, "Клубный администратор может редактировать только свой клуб");
    }
    ...
    if (body.logoUrl !== undefined) {
      if (body.logoUrl && !String(body.logoUrl).startsWith("/api/media/")) throw new HttpError(422, "Логотип загружается через медиатеку");
