...
import { audit, deleteMatchCascade } from "@/lib/engine/lifecycle";
import { assertRefereeAssignmentAllowed } from "@/lib/engine/conflicts";
import { assertSeasonInScope, assertMatchInScope, assertCanTouchFriendly } from "@/lib/scope";
...
export async function GET(req: Request) {
  try {
    ...
    // Скоуп-админ лиги работает только внутри своей лиги: товарищеские — вне скоупа
    if (isFriendlyList && user.role === "LEAGUE_ADMIN" && user.leagueId) {
      return Response.json({ matches: [] });
    }
    if (!isFriendlyList) await assertSeasonInScope(user, seasonId);
...
export async function POST(req: Request) {
  try {
    ...
    // ---------- Массовое удаление (v1.0.32: ПОЛНЫЙ каскад, без «пропущено») ----------
    // Раньше матчи с протоколом пропускались с тупиком «сначала Reset и
    // очистка» — теперь удаляются каскадом: события/составы/бригада/оценки
    // уходят вместе с матчем, игроки/команды/судьи/заявки — остаются,
    // таблица/дисциплина пересчитываются. UI предупреждает о необратимости.
    if (body.action === "bulk-delete") {
      const ids: string[] = Array.isArray(body.ids) ? body.ids.map(String) : [];
      if (ids.length === 0) throw new HttpError(422, "Ничего не выбрано");
      const deleted: { id: string; label: string; hadProtocol: boolean }[] = [];
      for (const matchId of ids) {
        await assertMatchInScope(user, matchId);
        ...
        if (!m) continue; // уже нет — не ошибка
        const hadProtocol = m._count.events > 0 || m._count.lineups > 0 || m._count.ratings > 0;
        await deleteMatchCascade(matchId, user);
        deleted.push({ id: matchId, label: `${m.homeTeam.name} — ${m.awayTeam.name}`, hadProtocol });
      }
      return Response.json({
        ok: true,
        deleted: deleted.length,
        withProtocol: deleted.filter((d) => d.hadProtocol).length,
        details: deleted,
      });
    }
    ...
    await assertSeasonInScope(user, stage.seasonId);
