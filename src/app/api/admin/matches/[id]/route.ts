  ...
  try {
    ...
    switch (action) {
      ...
      case "lineup": {
        ...
        await audit(user, "Lineup", `${id}:${teamId}`, "UPDATE", old.map((l) => ({ personId: l.personId, isStarter: l.isStarter })), personIds.map((p) => ({ personId: p, isStarter: starterSet.has(p) })));
        return Response.json({ ok: true, starters: starterSet.size, bench: personIds.length - starterSet.size });
      }
