...
import { validateEvent, completeMatch, resetMatch, assignWalkover, computeScore, audit, getEligiblePlayers, isRegisteredOn, deleteMatchCascade } from "@/lib/engine/lifecycle";
import { assertNotSuspended } from "@/lib/engine/discipline";
import { assertRefereeAssignmentAllowed } from "@/lib/engine/conflicts";
import { assertMatchInScope } from "@/lib/scope";
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await ctx.params;
    await assertMatchInScope(user, id);
...
// ---------- Удаление матча (v1.0.32: ПОЛНЫЙ каскад — любой матч) ----------
// Раньше 409 «сначала очистите протокол» завел пользователя в тупик
// (инструмента очистки не существовало). Теперь: матч с протоколом
// удаляется так же свободно — с явным предупреждением в UI о том, что
// именно уйдёт (события/составы/бригада/оценки/дисциплина) и что
// останется (игроки/команды/судьи/стадионы/заявки).
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    const user = await requireRole("LEAGUE_ADMIN", "SUPER_ADMIN");
    const { id } = await ctx.params;
    await assertMatchInScope(user, id);
    const report = await deleteMatchCascade(id, user);
    return Response.json({ ok: true, removed: report });
  } catch (e) {
    return errorResponse(e);
  }
}
