...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    switch (action) {
      ...
      case "lineup": {
        const { teamId, personIds, starters } = body as { teamId: string; personIds: string[]; starters?: string[] };
        ...
        await db.$transaction([
          ...
          db.lineupEntry.createMany({
            data: uniqueIds.map((personId, i) => ({
              matchId: id, teamId, personId, isStarter: starterSet.has(personId),
              number: regNumbers.get(personId) ?? i + 1,
            })),
          }),
        ]);
        await audit(user, "Lineup", `${id}:${teamId}`, "UPDATE", old.map((l) => ({ personId: l.personId, isStarter: l.isStarter })), uniqueIds.map((p) => ({ personId: p, isStarter: starterSet.has(p) })));
        return Response.json({ ok: true, starters: starterSet.size, bench: uniqueIds.length - starterSet.size });
      }
