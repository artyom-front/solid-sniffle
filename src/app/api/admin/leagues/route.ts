...
import { FORMAT_CODE_RE } from "@/lib/labels";
...
export function leaguePayload(body: Record<string, unknown>) {
  ...
  // v1.0.29: формат — любой код из админ-списка видов футбола (FormatLink):
  // базовая четвёрка + кастомные (F7, LFL_8…). Формат без ссылки в меню
  // просто сгруппируется в «Другие форматы» сайдбара.
  const format = String(body.format ?? "").trim().toUpperCase();
  if (!FORMAT_CODE_RE.test(format)) throw new HttpError(422, "Формат: код вида футбола из раздела «Сайт → Виды футбола» (например F11, F8, FUTSAL)");
...
export async function GET() {
  try {
    ...
    // v1.0.32: скоуп-админ видит только свою лигу
    const where = isLeagueScoped(user) ? { id: user.leagueId! } : {};
...
export async function POST(req: Request) {
  try {
    const user = await requireRole("LEAGUE_ADMIN", "SUPER_ADMIN");
    // создание лиг — привилегия оператора всего сайта/супер-админа
    if (isLeagueScoped(user)) throw new HttpError(403, "Создание новых лиг доступно супер-администратору или оператору сайта");
