...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    // v1.0.32: скоуп-админ правит ТОЛЬКО свою лигу
    if (isLeagueScoped(user) && id !== user.leagueId) throw new HttpError(403, "Это не ваша лига");
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    const user = await requireRole("LEAGUE_ADMIN", "SUPER_ADMIN");
    const { id } = await ctx.params;
    // удаление лиги — решение уровня сайта: скоуп-админу недоступно
    if (isLeagueScoped(user)) throw new HttpError(403, "Удаление лиг доступно супер-администратору или оператору сайта");
