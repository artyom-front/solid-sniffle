...
export async function POST(req: Request) {
  try {
    ...
    switch (action) {
      ...
      case "setup": {
        // v1.0.28 (аудит Task 27 🟡-6): симметрия с disable/recovery —
        // перегенерация секрета (сбрасывает включённую 2FA) только с
        // паролем: украденная сессия не может тихо ослабить защиту
        if (!body.password || !verifyPassword(String(body.password), full?.passwordHash ?? "")) {
          bumpFail(user.id);
          throw new HttpError(401, "Введите пароль: перегенерация секрета отключает включённую 2FA");
        }
