...
import { isTotpRequired } from "@/lib/settings";
...
export async function POST(req: Request) {
  try {
    ...
    // админ глобально выключил TOTP-логин — челленджи больше не принимаются
    // (login при этом сразу выдаёт сессию по паролю)
    if (!(await isTotpRequired())) {
      throw new HttpError(403, "Вход по коду отключён администратором — войдите заново по паролю");
    }
