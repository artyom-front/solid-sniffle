...
export async function POST(req: Request) {
  try {
    ...
    // учётка деактивирована администратором (панель «Пользователи»)
    if (user.isActive === false) {
      throw new HttpError(403, "Учётная запись отключена администратором. Обратитесь к администратору портала");
    }
