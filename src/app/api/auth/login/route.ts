...
export async function POST(req: Request) {
  try {
    ...
    // Глобальный тумблер админа: totp_required=0 — шаг 2FA пропускается
    // для всех пользователей (вход сразу по паролю).
    if (user.totpEnabled && user.totpSecret && (await isTotpRequired())) {
