...
export async function revertMatchDiscipline(matchId: string, tx: Prisma.TransactionClient = db) {
  ...
  const reds = await tx.suspension.findMany({
  ...
  const servedSnapshot = new Map<string, number>();
  const autoYellows = await tx.suspension.findMany({
    where: { seasonId: match.stage.seasonId, source: "AUTO_YELLOW" },
    select: { personId: true, matchesServed: true },
  });
  for (const s of autoYellows) {
    const prev = servedSnapshot.get(s.personId) ?? 0;
    if (s.matchesServed > prev) servedSnapshot.set(s.personId, s.matchesServed);
  }
  // пересчитываем накопление ЖК (удалив/пересоздав авто-жёлтые баны)
  await tx.suspension.deleteMany({ where: { seasonId: match.stage.seasonId, source: "AUTO_YELLOW" } });
  await processYellowAccrual(match.stage.seasonId, tx, servedSnapshot);
}

async function processYellowAccrual(seasonId: string, tx: Prisma.TransactionClient = db, servedSnapshot?: Map<string, number>) {
