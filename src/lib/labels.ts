...
/** Fallback-набор видов футбола, пока в БД нет строк FormatLink
 *  (база без сида миграции 00000000000002 или таблица очищена).
 *  Форма та же, что у публичного DTO: [code, label] по возрастанию порядка. */
export const DEFAULT_FORMAT_LINKS: { code: string; label: string }[] = [
  { code: "F11", label: "Футбол" },
  { code: "F8", label: "8×8" },
  { code: "F6", label: "6×6" },
  { code: "FUTSAL", label: "Мини-футбол" },
];

/** Валидный код формата: 1–12 символов A-Z/0-9/_ (совпадает с League.format).
 *  Используется и админ-API форматов/лиг, и публичной лентой (?format=F8). */
export const FORMAT_CODE_RE = /^[A-Z][A-Z0-9_]{0,11}$/;
