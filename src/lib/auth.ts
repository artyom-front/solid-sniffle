...
/** Тип одноразовой ссылки. invite — первичное приглашение (48 ч, чтобы
 *  человек успел получить письмо/сообщение и активироваться), pwset —
 *  сброс пароля (15 мин: «горячая» ссылка, короткий срок жизни снижает
 *  риск утечки). Одноразовость у обоих одинаковая — версионная (v). */
export type OneTimeLinkKind = "invite" | "pwset";

export const INVITE_TTL_MINUTES = 48 * 60;
export const PWSET_TTL_MINUTES = 15;
...
export function passwordResetToken(userId: string, sessionVersion: number): string {
  return signToken({ uid: userId, kind: "pwset", v: sessionVersion }, PWSET_TTL_MINUTES * 60);
}

/** v1.0.33 · Токен-приглашение: 48 часов (письмо может ждать прочтения),
 *  одноразовый — та же версионная механика. Уходит ТОЛЬКО на email
 *  пользователя (SMTP-режим) либо передаётся лично (ручной режим). */
export function inviteToken(userId: string, sessionVersion: number): string {
  return signToken({ uid: userId, kind: "invite", v: sessionVersion }, INVITE_TTL_MINUTES * 60);
}

/** Проверка токена приглашения/сброса: { uid, v, kind } или null (истёк/подделан) */
export function verifyOneTimeLinkToken(token: string): { uid: string; v: number; kind: OneTimeLinkKind } | null {
  const payload = verifyToken(token);
  if (!payload || (payload.kind !== "pwset" && payload.kind !== "invite")) return null;
  if (typeof payload.uid !== "string" || typeof payload.v !== "number") return null;
  return { uid: payload.uid, v: payload.v, kind: payload.kind };
}
