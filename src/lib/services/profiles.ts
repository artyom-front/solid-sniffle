...
export async function getMatchDetail(id: string) {
  const match = await db.match.findUnique({
    ...
    include: {
      ...
      events: {
        ...
        orderBy: [{ minute: "asc" }, { stoppage: { sort: "asc", nulls: "first" } }],
      },
  ...
  // Товарищеский матч: без сезона → без таблиц/H2H/сигналов
  const isFriendly = !match.stage;
  const seasonId = match.stage?.seasonId ?? null;
  const league = match.stage?.season.league ?? { id: "", name: "Товарищеский матч", walkoverScore: 3, yellowCardLimit: 99, format: "FRIENDLY" };

  // Роли заявки (PLAYER/COACH/…): для группировки состава на карточке —
  // «Стартовые / Запасные / Штаб». Товарищеский матч — активная заявка
  // на дату матча (сезона нет); турнирный — заявка сезона.
  const regRoleMap = new Map<string, string>();
  const roleRegs = await db.registration.findMany({
    where: seasonId
      ? { teamId: { in: [match.homeTeamId, match.awayTeamId] }, seasonId }
      : {
    ...
    select: { teamId: true, personId: true, role: true },
  });
  for (const r of roleRegs) {
    const key = `${r.teamId}:${r.personId}`;
    if (!regRoleMap.has(key)) regRoleMap.set(key, r.role);
  }

  // ---------- Таблица сезона (обе команды подсвечиваются на клиенте) ----------
  const standings = seasonId ? await seasonStandings(seasonId) : [];
  ...
  const h2hMatches = !isFriendly ? await db.match.findMany({
    where: {
  ...
  for (const m of h2hMatches) {
    ...
    const h2hWalkoverScore = m.stage?.season.league.walkoverScore ?? 3;
    const reg = m.status === "WALKOVER" && m.walkoverType
      ? m.walkoverType === "BOTH"
        ? { h: 0, a: 0 }
        : m.walkoverType === "HOME"
          ? { h: 0, a: h2hWalkoverScore }
          : { h: h2hWalkoverScore, a: 0 }
  ...
  if (seasonId && (match.status === "SCHEDULED" || match.status === "POSTPONED" || match.status === "LIVE")) {
    const teams = [
  ...
  const sigCtx = seasonId ? await buildSignalsContext(seasonId) : null;
  const signals = sigCtx
    ? matchSignals(
        { id: match.id, round: match.round, homeTeamId: match.homeTeamId, awayTeamId: match.awayTeamId, homeName: match.homeTeam.name, awayName: match.awayTeam.name },
        sigCtx,
        standings
      )
    : null;
  ...
  return {
    match: {
      ...
      // city: населённый пункт команды (герой карточки: подпись под названием
      // вместо «хозяева/гости»); у команды без города — город клуба
      homeTeam: { id: match.homeTeam.id, name: match.homeTeam.name, clubName: match.homeTeam.club?.name ?? null, city: match.homeTeam.city ?? match.homeTeam.club?.city ?? null },
      awayTeam: { id: match.awayTeam.id, name: match.awayTeam.name, clubName: match.awayTeam.club?.name ?? null, city: match.awayTeam.city ?? match.awayTeam.club?.city ?? null },
      ...
      league: { id: league.id, name: league.name, walkoverScore: league.walkoverScore, yellowCardLimit: league.yellowCardLimit, format: league.format },
      ...
      lineups: match.lineups.map((l) => ({
        ...
        number: l.number,
        regRole: regRoleMap.get(`${l.teamId}:${l.personId}`) ?? "PLAYER",
      })),
    ...
    h2h: {
      list: h2hMatches.map((m) => ({
        ...
        regulationScore: m.stage?.season.league.walkoverScore ?? 3,
        season: m.stage ? { name: m.stage.season.name, league: m.stage.season.league.name } : { name: "—", league: "Товарищеский матч" },
      })),
...
export async function getTeamProfile(id: string) {
  ...
  return {
    ...
    matches: teamMatches.map((m) => {
      const lg = m.stage?.season.league;
      return {
        ...toMatchDTO(m, lg?.walkoverScore ?? 3),
        league: lg ? { id: lg.id, name: lg.name, format: lg.format } : { id: "friendly", name: "Товарищеские матчи", format: "FRIENDLY" },
      };
    }),
  };
}
...
export async function getPlayerProfile(id: string) {
  ...
  // ---------- Тренерская карьера (отдельно от игровой и судейской) ----------
  // Главный тренер = заявка Registration.role=COACH. Считаем матчи команды
  // за период заявки: В-Н-П, мячи, очки. Играющая карьера и судейская
  // карьера считаются НЕЗАВИСИМО — «каши» нет (инвариант «судья ≠ игрок»).
  interface CoachSeasonRow {
    season: { id: string; name: string; league: string };
    team: { id: string; name: string };
    matches: number;
    wins: number;
  ...
  const coachSeasons: CoachSeasonRow[] = [];
  const coachRegs = person.registrations.filter((r) => r.role === "COACH");
  for (const reg of coachRegs) {
    const teamMatches = await db.match.findMany({
      where: {
        stage: { seasonId: reg.seasonId },
        status: "COMPLETED",
        kickoff: { gte: reg.startDate, ...(reg.endDate ? { lte: reg.endDate } : {}) },
        OR: [{ homeTeamId: reg.teamId }, { awayTeamId: reg.teamId }],
      },
    });
    let wins = 0, draws = 0, losses = 0, goalsFor = 0, goalsAgainst = 0;
    for (const m of teamMatches) {
      const isHome = m.homeTeamId === reg.teamId;
      const my = (isHome ? m.homeScore : m.awayScore) ?? 0;
      const opp = (isHome ? m.awayScore : m.homeScore) ?? 0;
      goalsFor += my;
      goalsAgainst += opp;
    ...
    coachSeasons.push({
      season: { id: reg.season.id, name: reg.season.name, league: reg.season.league.name },
      team: { id: reg.team.id, name: reg.team.name },
      matches: teamMatches.length,
      wins, draws, losses, goalsFor, goalsAgainst,
      points: wins * 3 + draws,
      startDate: reg.startDate.toISOString(),
      endDate: reg.endDate?.toISOString() ?? null,
    });
  }
  const coachTotals = coachSeasons.reduce(
    (a, s) => ({
      matches: a.matches + s.matches,
      wins: a.wins + s.wins,
      draws: a.draws + s.draws,
      losses: a.losses + s.losses,
      goalsFor: a.goalsFor + s.goalsFor,
      goalsAgainst: a.goalsAgainst + s.goalsAgainst,
    }),
  ...
  if (person.isReferee) {
    ...
    for (const m of refMatches) {
      ...
      const key = m.stage?.season.league.name ?? "Товарищеские матчи";
    ...
    referee = {
      ...
      matchList: refMatches.slice(0, 30).map((m) => ({
        ...
        league: m.stage?.season.league.name ?? "Товарищеские матчи",
