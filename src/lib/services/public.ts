...
import { DEFAULT_FORMAT_LINKS, FORMAT_CODE_RE } from "@/lib/labels";

/** Порог «разгрома» для превью: 10+ мячей в одном матче */
export const ROUT_THRESHOLD = 10;

/** Пустой контекст сигналов — для товарищеских матчей (без турнирной статистики) */
const emptySignalsContext: Parameters<typeof matchSignals>[1] = {
  standings: [],
  matches: [],
  topScorers: new Map(),
  suspended: new Map(),
...
export async function getOverview() {
  const [leagues, formatLinks] = await Promise.all([
    db.league.findMany({
      include: { seasons: { orderBy: { startDate: "desc" } } },
      orderBy: { createdAt: "asc" },
    }),
    db.formatLink.findMany({ where: { isVisible: true }, orderBy: { sortOrder: "asc" } }),
  ]);
  ...
  return {
    // виды футбола для меню сайта: управляются админом (v1.0.29);
    // пусто в БД (без сида) — откат к базовой четвёрке
    formats:
      formatLinks.length > 0
        ? formatLinks.map((f) => ({ code: f.code, label: f.label, sortOrder: f.sortOrder }))
        : DEFAULT_FORMAT_LINKS.map((f, i) => ({ ...f, sortOrder: (i + 1) * 10 })),
...
export async function getBanners() {
  ...
  return {
    banners: banners.map((b) => ({
      ...
      markSize: b.markSize, markOpacity: b.markOpacity,
    })),
  };
}

// ============================================================
// Стат-карточки правой колонки (v1.0.29)
// ============================================================

/** Активные стат-карточки для правой колонки сайта (SSR — без мигания).
 *  Карточки рендерятся боксом фиксированной высоты — анти-CLS. */
export async function getStatBlocks() {
  const blocks = await db.statBlock.findMany({
    where: { isActive: true },
    orderBy: [{ priority: "asc" }, { createdAt: "desc" }],
  });
  return {
...
// ============================================================
// Виды футбола (меню сайта, v1.0.29)
// ============================================================

/** Видимые виды футбола (админка: FormatLink), по порядку сортировки.
 *  Пусто в БД — откат к базовой четвёрке. Одна форма для SSR и API. */
export async function getFormatLinks() {
  const links = await db.formatLink.findMany({ where: { isVisible: true }, orderBy: { sortOrder: "asc" } });
  const formats =
    links.length > 0
      ? links.map((f) => ({ code: f.code, label: f.label, sortOrder: f.sortOrder }))
      : DEFAULT_FORMAT_LINKS.map((f, i) => ({ ...f, sortOrder: (i + 1) * 10 }));
  return { formats };
}
...
export async function getMatchesDay(date: string, format: string) {
  // формат — валидный код (совпадает с League.format); «all» — без фильтра.
  // Раньше был хардкод-белый список F11/F8/F6/FUTSAL — новые форматы из
  // админки (v1.0.29) через него не проходили и молча включали ВСЕ лиги.
  const wantFormat = format !== "all" && FORMAT_CODE_RE.test(format);
  const leagues = await db.league.findMany({
    where: wantFormat ? { format } : {},
    ...
    result.push({
      ...
      matches: matches.map((m) => ({
        ...
        signals: matchSignals(
          { id: m.id, round: m.round, homeTeamId: m.homeTeamId, awayTeamId: m.awayTeamId, homeName: m.homeTeam.name, awayName: m.awayTeam.name },
          ctx,
          standings
        ),
      })),
  ...
  // ---------- Товарищеские матчи этого дня (вне лиг/сезонов) ----------
  if (start && end) {
    const friendlies = await db.match.findMany({
      where: { isFriendly: true, kickoff: { gte: start, lt: end } },
      include: {
        homeTeam: { include: { club: true } },
        awayTeam: { include: { club: true } },
        stadium: true,
    ...
    if (friendlies.length > 0) {
      result.push({
        league: { id: "friendly", name: "Товарищеские матчи", shortName: "Товарищеские", format: "FRIENDLY", isPinned: false, walkoverScore: 3 },
        season: { id: "", name: "" },
        matches: friendlies.map((m) => ({
          ...toMatchDTO(m, 3),
          signals: matchSignals(
            { id: m.id, round: m.round, homeTeamId: m.homeTeamId, awayTeamId: m.awayTeamId, homeName: m.homeTeam.name, awayName: m.awayTeam.name },
            emptySignalsContext,
...
export async function getStadiumProfile(id: string) {
  ...
  // товарищеские матчи (без этапа) — регламентный счёт 3:0 по умолчанию
  const leagueOf = (m: (typeof matches)[number]) =>
    m.stage?.season.league ?? { id: "friendly", name: "Товарищеские матчи", format: "FRIENDLY", walkoverScore: 3 };

  return {
    stadium: { id: stadium.id, name: stadium.name, city: stadium.city, address: stadium.address, capacity: stadium.capacity, photoUrl: stadium.photoUrl },
    stats: { hosted: played.length, goals, avgGoals: played.length ? +(goals / played.length).toFixed(1) : 0 },
    matches: matches.map((m) => {
      const lg = leagueOf(m);
      return {
        ...toMatchDTO(m, lg.walkoverScore),
