...
export function toMatchDTO(m: Awaited<ReturnType<typeof loadSeasonData>>["matches"][number] | {
  ...
  return {
    ...
    homeTeam: { id: m.homeTeam.id, name: m.homeTeam.name, clubName: (m.homeTeam as { club?: { name: string | null } | null }).club?.name ?? null, logoUrl: (m.homeTeam as { logoUrl?: string | null }).logoUrl ?? null },
    awayTeam: { id: m.awayTeam.id, name: m.awayTeam.name, clubName: (m.awayTeam as { club?: { name: string | null } | null }).club?.name ?? null, logoUrl: (m.awayTeam as { logoUrl?: string | null }).logoUrl ?? null },
