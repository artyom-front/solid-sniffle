...
const PLACEMENTS: { code: string; label: string; size: string }[] = [
  { code: "TOP", label: "Верх — полоса под шапкой", size: "800×90" },
  { code: "BOTTOM", label: "Низ — полоса над футером", size: "800×90" },
  { code: "RIGHT_TOP", label: "Виджеты · верхний блок", size: "300×250" },
  { code: "RIGHT_BOTTOM", label: "Виджеты · нижний блок", size: "300×250" },
...
function BannerPreview({ form }: { form: BannerForm }) {
  ...
  const isRail = form.placement === "RIGHT_TOP" || form.placement === "RIGHT_BOTTOM";
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
