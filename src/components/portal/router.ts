...
export function HashRedirect() {
  ...
  useEffect(() => {
    ...
    if (KNOWN_HASH_ROUTES.includes(head)) {
      ...
      // Фолбэк легитимен: эффект HashRedirect (чайлд) срабатывает РАНЬШЕ
      // эффекта SiteShell (родитель), где bindRouter — на этом тике роутер
      // ещё не привязан, полный переход браузера единственный корректный путь.
