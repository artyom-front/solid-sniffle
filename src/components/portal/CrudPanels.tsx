...
import { FORMAT_LABELS, DEFAULT_FORMAT_LINKS } from "./types";
...
export function TournamentsPanel({ bump, onReload, version = 0, onNavigate }: CrudProps) {
  const { data, loading } = useFetch<{ leagues: AdminLeague[] }>("/api/admin/leagues", version);
  // форматы — из админ-списка видов футбола (v1.0.29): видимые и скрытые;
  // пусто (нет сида/новая база) — базовая четвёрка
  const { data: fmtData } = useFetch<{ formats: { code: string; label: string }[] }>("/api/admin/formats", version);
  const fmtOptions = fmtData?.formats?.length ? fmtData.formats : DEFAULT_FORMAT_LINKS;
  ...
  const saveLeague = async () => {
    ...
    const res = await apiPost<{ league?: { id: string } }>(leagueForm.id ? `/api/admin/leagues/${leagueForm.id}` : "/api/admin/leagues", body, leagueForm.id ? "PATCH" : "POST");
    setSaving(false);
    if (!res.ok) return toast.error(res.error);
    if (!leagueForm.id) {
      toast.success("Лига создана — теперь добавьте ей сезон");
      if (res.data?.league?.id) setExpandedId(res.data.league.id);
    } else {
      toast.success("Лига обновлена");
    }
    setLeagueForm(null);
    bump();
    onReload();
  ...
  const saveSeason = async () => {
    ...
    if (!res.ok) return toast.error(res.error);
    if (!seasonForm.id) {
      toast.success("Сезон создан · дальше: заявите команды и сгенерируйте расписание (раздел «Расписание»)");
      setExpandedId(seasonForm.leagueId);
    } else {
    ...
    <div className="space-y-4">
      ...
      {leagues.length === 0 && (
        <div className="rounded-xl border border-dashed border-emerald-300 bg-emerald-50/50 p-4">
          <p className="mb-2 text-sm font-bold text-emerald-800">Как запустить чемпионат — 4 шага</p>
          <ol className="list-decimal space-y-1 pl-5 text-xs text-emerald-700">
            <li><b>Создайте лигу</b> (кнопка сверху) — чемпионат: название, формат, правила</li>
            <li>В карточке лиги нажмите <b>«Сезоны» → «Добавить сезон»</b> — без сезона матчи не создать</li>
            <li>Заявите команды: <b>Справочники → Клубы и команды</b>, затем <b>Заявки и трансферы</b> (или массово через <b>Импорт</b>)</li>
            <li>Раздел <b>«Расписание»</b> — генератор календаря: круговая система за один клик (или создавайте матчи вручную в разделе «Матчи»)</li>
          </ol>
          <p className="mt-2 text-xs text-emerald-600">Товарищеские матчи создаются без лиги: раздел «Матчи» → переключатель «Товарищеские».</p>
        </div>
      )}
      {leagues.map((l) => (
        <div key={l.id} className="rounded-xl border border-zinc-200 bg-white">
          ...
          {expandedId === l.id && (
            <div className="space-y-1.5 px-4 py-3">
              {l.seasons.length === 0 && (
                <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">У лиги пока нет сезонов — добавьте сезон, иначе матчи создать нельзя</p>
              )}
              {l.seasons.map((s) => (
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-zinc-500 hover:text-emerald-600" onClick={() => onNavigate?.("registrations")}>
                  Заявить игроков →
                </Button>
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-zinc-500 hover:text-emerald-600" onClick={() => onNavigate?.("schedule")}>
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          {leagueForm && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Формат" hint="Список ведётся в «Сайт → Виды футбола»">
                <select value={leagueForm.format} onChange={(e) => setLeagueForm({ ...leagueForm, format: e.target.value })} className="h-9 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm">
                  {fmtOptions.map((f) => <option key={f.code} value={f.code}>{f.label} · {f.code}</option>)}
                  {/* легаси-формат лиги, которого нет в списке — показываем, чтобы не потерять */}
                  {!fmtOptions.some((f) => f.code === leagueForm.format) && (
                    <option value={leagueForm.format}>{FORMAT_LABELS[leagueForm.format] ?? leagueForm.format} · {leagueForm.format}</option>
                  )}
                </select>
              </Field>
...
interface AdminClub { id: string; name: string; city: string | null; description: string | null; logoUrl: string | null; teams: { id: string; name: string }[]; teamsCount: number }
interface AdminTeam { id: string; name: string; city: string | null; logoUrl: string | null; club: { id: string; name: string } | null; registrationsCount: number; matchesCount: number }

export function ClubsTeamsPanel({ bump, onReload, version = 0, onOpenTeam, userRole }: CrudProps & { onOpenTeam?: (teamId: string) => void; userRole?: string }) {
  const [tab, setTab] = useState<"teams" | "clubs">("teams");
  const { data: teamsData, loading: teamsLoading } = useFetch<{ teams: AdminTeam[] }>("/api/admin/teams", version);
  const { data: clubsData, loading: clubsLoading } = useFetch<{ clubs: AdminClub[] }>("/api/admin/clubs", version);
  const [teamForm, setTeamForm] = useState<{ id?: string; name: string; clubId: string; city: string; logoUrl: string } | null>(null);
  const [clubForm, setClubForm] = useState<{ id?: string; name: string; city: string; description: string; logoUrl: string } | null>(null);
  const [saving, setSaving] = useState(false);
  ...
  const saveTeam = async () => {
    if (!teamForm) return;
    setSaving(true);
    const res = await apiPost(teamForm.id ? `/api/admin/teams/${teamForm.id}` : "/api/admin/teams", { name: teamForm.name, clubId: teamForm.clubId || null, city: teamForm.city || null, logoUrl: teamForm.logoUrl || null }, teamForm.id ? "PATCH" : "POST");
    setSaving(false);
    if (!res.ok) return toast.error(res.error);
  ...
  const saveClub = async () => {
    if (!clubForm) return;
    setSaving(true);
    const res = await apiPost(clubForm.id ? `/api/admin/clubs/${clubForm.id}` : "/api/admin/clubs", { name: clubForm.name, city: clubForm.city, description: clubForm.description, logoUrl: clubForm.logoUrl || null }, clubForm.id ? "PATCH" : "POST");
    setSaving(false);
    if (!res.ok) return toast.error(res.error);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-zinc-400">Клик по команде — карточка со составом, фото и матчами · клуб (бренд/юрлицо) ≠ команда (состав в лиге)</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {tab === "teams" ? (
            <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setTeamForm({ name: "", clubId: "", city: "", logoUrl: "" })}>
              <Plus className="mr-1 h-4 w-4" /> Команда
            </Button>
          ) : (
            <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setClubForm({ name: "", city: "", description: "", logoUrl: "" })}>
      ...
      {tab === "teams" && (teamsLoading && !teamsData ? <LoadingBlock /> : (
        <>
          ...
          <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
            {teams.length === 0 && <p className="px-4 py-8 text-center text-sm text-zinc-400">Команд нет — создайте первую или импортируйте списком (раздел «Импорт»)</p>}
            {teams.map((t) => (
              <div key={t.id} className={cn("group flex flex-wrap items-center gap-2 border-b border-zinc-50 px-4 py-2.5 last:border-b-0 hover:bg-zinc-50/60", selected.has(t.id) && "bg-emerald-50/50")}>
                {canDelete && (
                  <RowCheckbox
                ...
                {t.logoUrl ? (
                  ...
                  <img src={t.logoUrl} alt="" className="h-8 w-8 rounded-lg border border-zinc-200 object-cover" />
                ) : null}
                <button
                  onClick={() => onOpenTeam?.(t.id)}
                  className="rounded font-semibold text-zinc-800 hover:bg-emerald-50 hover:text-emerald-700"
                  title="Открыть карточку команды: состав, фото, матчи"
                >
                  {t.name}
                </button>
                ...
                <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-400 opacity-0 transition-opacity group-hover:opacity-100 hover:text-emerald-600" onClick={() => setTeamForm({ id: t.id, name: t.name, clubId: t.club?.id ?? "", city: t.city ?? "", logoUrl: t.logoUrl ?? "" })} aria-label="Быстрое редактирование">
                  <Pencil className="h-3.5 w-3.5" />
                </Button>
                {canDelete ? (
                  <SmartDelete
                    endpoint={`/api/admin/teams/${t.id}`}
                    entityLabel={`Команда «${t.name}»`}
                    title={`Удаление команды «${t.name}»`}
                    onDone={() => { bump(); onReload(); }}
                    onOpenDetail={onOpenTeam ? () => onOpenTeam(t.id) : undefined}
                  />
                ) : null}
              </div>
            ))}
          ...
          {clubs.map((c) => (
            <div key={c.id} className="flex flex-wrap items-center gap-2 border-b border-zinc-50 px-4 py-2.5 last:border-b-0">
              {c.logoUrl ? (
                ...
                <img src={c.logoUrl} alt="" className="h-8 w-8 rounded-lg border border-zinc-200 object-cover" />
              ) : (
              ...
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-400 hover:text-emerald-600" onClick={() => setClubForm({ id: c.id, name: c.name, city: c.city ?? "", description: c.description ?? "", logoUrl: c.logoUrl ?? "" })} aria-label="Редактировать">
                <Pencil className="h-3.5 w-3.5" />
              </Button>
              <DeleteBtn onClick={async () => {
      ...
      <Dialog open={!!teamForm} onOpenChange={(o) => !o && setTeamForm(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{teamForm?.id ? "Быстрое редактирование команды" : "Новая команда"}</DialogTitle></DialogHeader>
          {teamForm && (
            <div className="space-y-3">
              {teamForm.id && (
                <p className="rounded-lg bg-zinc-50 px-3 py-2 text-xs text-zinc-500">
                  Это быстрое редактирование. Полная карточка — по клику на название команды в списке: там состав, массовое добавление игроков и матчи.
                </p>
              )}
              ...
              <Field label="Клуб (необязательно)" hint="Вышестоящая организация: у одного клуба может быть несколько команд — основная, дубль, ветераны. Клуб = бренд/юрлицо, команда = состав в лиге">
                <select value={teamForm.clubId} onChange={(e) => setTeamForm({ ...teamForm, clubId: e.target.value })} className="h-9 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm">
                  <option value="">— без клуба (самостоятельная команда) —</option>
              ...
              <MediaUpload value={teamForm.logoUrl} onChange={(url) => setTeamForm({ ...teamForm, logoUrl: url })} label="Эмблема команды" hint="Квадратная картинка — показывается на странице команды" />
            </div>
          )}
          <DialogFooter>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {clubForm && (
            <div className="space-y-3">
              ...
              <MediaUpload value={clubForm.logoUrl} onChange={(url) => setClubForm({ ...clubForm, logoUrl: url })} label="Эмблема клуба" hint="Общая для всех команд клуба" />
            </div>
          )}
          <DialogFooter>
