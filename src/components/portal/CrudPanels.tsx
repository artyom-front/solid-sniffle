...
export function ClubsTeamsPanel({ bump, onReload, version = 0 }: CrudProps) {
        ...
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
          {teams.map((t) => (
            <div key={t.id} className="flex flex-wrap items-center gap-2 border-b border-zinc-50 px-4 py-2.5 last:border-b-0">
              ...
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-400 hover:text-emerald-600" onClick={() => setTeamForm({ id: t.id, name: t.name, clubId: t.club?.id ?? "", city: t.city ?? "", logoUrl: t.logoUrl ?? "" })} aria-label="Редактировать">
                <Pencil className="h-3.5 w-3.5" />
              </Button>
              <DeleteBtn onClick={async () => {
