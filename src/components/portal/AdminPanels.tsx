...
const REGISTRATION_ROLE_OPTIONS = [
...
interface RegData {
  persons: { id: string; name: string; position: string | null; isReferee?: boolean; teams: string[] }[];
}

interface RegListItem {
  id: string; personId: string; personName: string; personPhoto: string | null;
  personPosition: string | null; personIsReferee: boolean;
...
export function RegistrationsPanel({ seasonId, bump, version = 0, onOpenTeam, onOpenPerson }: {
  seasonId: string; bump: () => void; version?: number;
  ...
  const { data } = useFetch<RegData>(seasonId ? `/api/admin/persons?seasonId=${seasonId}` : null, version);
  const { data: teamsData } = useFetch<{ teams: { id: string; name: string }[] }>(seasonId ? `/api/public/teams?seasonId=${seasonId}` : null, version);
  const { data: regsData, loading: regsLoading } = useFetch<{ registrations: RegListItem[] }>(seasonId ? `/api/admin/registrations?seasonId=${seasonId}` : null, version);
  ...
  // подсказка: судья + заявка игроком — сработает блок на уровне API, предупредим заранее
  const refereeWarn = selectedPerson?.isReferee && role === "PLAYER";

  async function submit() {
    if (!personId || !teamId) return toast.error("Выберите игрока и команду");
    setBusy(true);
    const res = await apiPost("/api/admin/registrations", {
      personId, teamId, seasonId, role, number: number ? Number(number) : null, endDatePrevious: endPrevious ? new Date().toISOString() : null,
    });
    setBusy(false);
  ...
  async function regAction(r: RegListItem, action: "end" | "reactivate" | "delete") {
    setRegBusy(r.id);
    const res =
      action === "end"
  ...
  // Список по командам сезона
  const regs = regsData?.registrations ?? [];
  const byTeam = new Map<string, RegListItem[]>();
  for (const r of regs) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          ...
          Трансферное окно лиги проверяется автоматически. Составы команд — в карточке команды (клик по названию ниже).
        </p>
      </div>
      <Card className="border-zinc-200">
        <CardContent className="grid gap-3 lg:grid-cols-4">
            ...
            <Select value={personId} onValueChange={setPersonId}>
              ...
              <SelectContent className="max-h-72">
                {persons.slice(0, 100).map((p) => (
                  <SelectItem key={p.id} value={p.id}>{p.name}{p.isReferee ? " · судья" : ""}{p.teams.length ? ` (${p.teams.join(", ")})` : " — без команды"}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-1">
            <Label className="text-xs">Роль в заявке</Label>
            <select value={role} onChange={(e) => setRole(e.target.value)} className="h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm">
              {REGISTRATION_ROLE_OPTIONS.map((r) => <option key={r.code} value={r.code}>{r.name}</option>)}
            </select>
          </div>
          <div className="space-y-1">
          ...
          {refereeWarn && (
            <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700 lg:col-span-4">
              Этот человек — судья. Заявка игроком будет отклонена, если он назначен на матчи ЭТОГО сезона. Играет и судит — только в разных чемпионатах.
            </p>
          )}
        </CardContent>
      </Card>

      {/* ---------- Составы сезона по командам ---------- */}
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h4 className="text-sm font-bold text-zinc-700">Составы этого сезона · {regs.length} заявок</h4>
          <p className="text-xs text-zinc-400">Клик по команде — карточка со составом; кнопки справа — отзаявить/вернуть/удалить</p>
        </div>
        {regsLoading && regs.length === 0 && <p className="py-4 text-center text-sm text-zinc-400">Загрузка...</p>}
        {!regsLoading && regs.length === 0 && (
          <p className="rounded-xl border border-dashed border-zinc-200 py-8 text-center text-sm text-zinc-400">
            В этом сезоне пока нет заявок. Оформите заявку выше или откройте карточку команды — там есть массовое добавление из Excel.
          </p>
        )}
        {[...byTeam.entries()].map(([tid, list]) => (
          <div key={tid} className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
            <div className="flex items-center gap-2 border-b border-zinc-100 bg-zinc-50/60 px-4 py-2">
              {onOpenTeam ? (
                <button onClick={() => onOpenTeam(tid)} className="rounded text-sm font-bold text-zinc-800 hover:bg-emerald-50 hover:text-emerald-700" title="Открыть карточку команды">
                  {list[0].teamName}
                </button>
              ) : (
                <span className="text-sm font-bold text-zinc-800">{list[0].teamName}</span>
              )}
              <Badge variant="secondary" className="font-normal">{list.filter((r) => !r.endDate).length} в заявке</Badge>
              {list.some((r) => r.endDate) && <Badge variant="outline" className="font-normal text-zinc-400">{list.filter((r) => r.endDate).length} закрыто</Badge>}
            </div>
            {list.map((r) => (
              <div key={r.id} className={cn("flex flex-wrap items-center gap-2 border-b border-zinc-50 px-4 py-2 last:border-b-0", r.endDate && "bg-zinc-50/50")}>
                <span className="w-7 text-center font-mono text-sm font-bold text-zinc-400">{r.number ?? "–"}</span>
                {r.personPhoto
                  ? <img src={r.personPhoto} alt="" className="h-7 w-7 rounded-full border border-zinc-200 object-cover" />
                  : <span className="flex h-7 w-7 items-center justify-center rounded-full bg-zinc-100 text-xs font-bold text-zinc-400">{r.personName.slice(0, 1)}</span>}
                {onOpenPerson ? (
                  <button onClick={() => onOpenPerson(r.personId)} className="rounded text-sm font-semibold text-zinc-800 hover:bg-emerald-50 hover:text-emerald-700" title="Открыть карточку игрока">
                    {r.personName}
                  </button>
                ) : (
                  <span className="text-sm font-semibold text-zinc-800">{r.personName}</span>
                )}
                {r.personIsReferee && <Badge variant="outline" className="font-normal text-amber-600">судья</Badge>}
                <Badge variant={r.role === "PLAYER" ? "secondary" : "outline"} className="font-normal">{REGISTRATION_ROLE_OPTIONS.find((o) => o.code === r.role)?.name ?? r.role}</Badge>
                <span className="ml-auto hidden text-xs text-zinc-400 md:inline">
                  {new Date(r.startDate).toLocaleDateString("ru-RU")}{r.endDate ? ` — ${new Date(r.endDate).toLocaleDateString("ru-RU")}` : ""}
                </span>
                {r.endDate && <Badge variant="outline" className="font-normal text-zinc-400">закрыта</Badge>}
                {regBusy === r.id ? (
...
// ============================================================
// Merge профилей (Epic 4) + Merge команд («правопреемник», v1.0.19)
// ============================================================

interface MergePersonData {
...
interface MergeTeamData {
  teams: { id: string; name: string; clubName: string | null; links: { matches: number; registrations: number; events: number } }[];
}

export function MergePanel({ bump }: { bump: () => void }) {
  const [mode, setMode] = useState<"persons" | "teams">("persons");
  const [query, setQuery] = useState("");
  const { data: personsData } = useFetch<MergePersonData>(mode === "persons" ? `/api/admin/merge?q=${encodeURIComponent(query)}` : null);
  const { data: teamsData } = useFetch<MergeTeamData>(mode === "teams" ? `/api/admin/merge?type=team&q=${encodeURIComponent(query)}` : null);
  const [fromId, setFromId] = useState("");
  const [toId, setToId] = useState("");
  const [busy, setBusy] = useState(false);

  const from = mode === "persons"
    ? personsData?.persons.find((p) => p.id === fromId)
    : undefined;
  const to = mode === "persons"
    ? personsData?.persons.find((p) => p.id === toId)
    : undefined;
  const fromTeam = mode === "teams" ? teamsData?.teams.find((t) => t.id === fromId) : undefined;
  const toTeam = mode === "teams" ? teamsData?.teams.find((t) => t.id === toId) : undefined;

  async function merge() {
    if (!fromId || !toId) return toast.error("Выберите обе сущности");
    if (fromId === toId) return toast.error("Выбранные записи совпадают");
    if (mode === "teams" && !confirm(
      `Объединить команды?\n\n«${fromTeam?.name}» будет удалена, вся её история (${fromTeam?.links.matches ?? 0} матчей, ${fromTeam?.links.registrations ?? 0} заявок) перейдёт команде «${toTeam?.name}» — она станет правопреемником.`
    )) return;
    setBusy(true);
    const res = await apiPost<{ transferred: Record<string, number> }>("/api/admin/merge", { fromId, toId, type: mode === "teams" ? "team" : "person" });
    setBusy(false);
    if (!res.ok) return toast.error(res.error, { duration: 8000 });
    const t = res.data?.transferred;
    if (mode === "teams") {
      toast.success(`Команды объединены: правопреемник получил ${t?.matches ?? 0} матчей, ${t?.registrations ?? 0} заявок, ${t?.events ?? 0} событий`);
    } else {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="flex items-center gap-2 font-semibold"><GitMerge className="h-4 w-4 text-emerald-600" /> Гигиена данных: слияние сущностей</h3>
        <p className="text-sm text-zinc-500">
          Переносит всю историю на выбранную запись и удаляет дубль — транзакционно, с записью в журнал аудита.
          Решает сценарий «сущность пересоздали»: статистика не теряется, а переезжает к правопреемнику.
        </p>
      </div>

      {/* режим: персоны / команды */}
      <div className="flex w-fit rounded-lg bg-zinc-100 p-0.5">
        {([["persons", "Профили (люди)"], ["teams", "Команды (правопреемник)"]] as const).map(([id, label]) => (
          <button key={id} onClick={() => { setMode(id); setFromId(""); setToId(""); }} className={cn("rounded-md px-3 py-1.5 text-xs font-semibold", mode === id ? "bg-white text-emerald-700 shadow-sm" : "text-zinc-500 hover:text-zinc-800")}>
            {label}
          </button>
        ))}
      </div>
          ...
          {mode === "persons" ? (
            <>
              <p className="text-xs text-zinc-500">
                ...
                все связи будут перепривязаны, дубликат удалён. Профиль игрока/судьи, пересозданный после удаления, «заберёт» свою прошлую статистику.
              </p>
                ...
                {([["Дубликат (будет удалён)", fromId, setFromId, from], ["Канонический профиль (останется)", toId, setToId, to]] as const).map(([label, value, setter, preview]) => (
                  <div key={label} className="space-y-1 rounded-xl border border-zinc-200 p-3">
                    ...
                    <Select value={value} onValueChange={setter}>
                      ...
                      <SelectContent className="max-h-72">
                        {(personsData?.persons ?? []).slice(0, 100).map((p) => (
                    ...
                    {preview && (
                      <div className="mt-2 rounded-lg bg-zinc-50 p-2 text-xs text-zinc-500">
                        ...
                        <p>Событий: {preview.links.events} · Заявок: {preview.links.registrations} · Дисквалификаций: {preview.links.suspensions} · Матчей судьи: {preview.links.refereedMatches}</p>
                      </div>
                    )}
                  </div>
                ))}
            ...
            <>
              <p className="text-xs text-zinc-500">
                Команда пересоздана под другим именем (реорганизация, переименование, новый сезон)? Новая команда — <b>правопреемник</b>:
                выберите старую команду (источник, будет удалена) и новую (цель, останется и получит всю историю — матчи, заявки, события протоколов).
                Команды, игравшие друг против друга, объединять нельзя.
              </p>
              <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Поиск по названию команды..." className="max-w-sm bg-white" />
              <div className="grid gap-3 md:grid-cols-2">
                {([["Старая команда (будет удалена)", fromId, setFromId, fromTeam], ["Правопреемник (получит историю)", toId, setToId, toTeam]] as const).map(([label, value, setter, preview]) => (
                  <div key={label} className="space-y-1 rounded-xl border border-zinc-200 p-3">
                    <Label className="text-xs font-semibold">{label}</Label>
                    <Select value={value} onValueChange={setter}>
                      <SelectTrigger className="bg-white"><SelectValue placeholder="Выбрать команду" /></SelectTrigger>
                      <SelectContent className="max-h-72">
                        {(teamsData?.teams ?? []).slice(0, 150).map((t) => (
                          <SelectItem key={t.id} value={t.id}>
                            {t.name} · {t.links.matches} матчей, {t.links.registrations} заявок
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    {preview && (
                      <div className="mt-2 rounded-lg bg-zinc-50 p-2 text-xs text-zinc-500">
                        <p className="font-medium text-zinc-700">{preview.name}{preview.clubName ? ` · ${preview.clubName}` : ""}</p>
                        <p>Матчей: {preview.links.matches} · Заявок: {preview.links.registrations} · Событий протоколов: {preview.links.events}</p>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <Button className="bg-emerald-600 hover:bg-emerald-700" disabled={busy || !fromId || !toId} onClick={merge}>
                <GitMerge className="mr-1 h-4 w-4" /> Объединить команды (правопреемник)
              </Button>
            </>
          )}
        </CardContent>
