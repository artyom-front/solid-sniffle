...
export default function MatchDayView({ format, version, initialDay }: Props) {
  ...
  /** Короткая подпись дня: «Сегодня», «Вчера», «Завтра» — только для соседних дней */
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-sline bg-s1">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-sline/60 px-4 py-3">
          {/* ЕДИНСТВЕННЫЙ фильтр даты: ‹ [Сегодня · 15 сентября] › + календарь.
              Кнопка с датой = возврат к сегодня; стрелки листают дни. */}
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className={cn(
                "flex min-w-44 items-baseline justify-center gap-1.5 rounded-lg px-3 py-1 text-xs font-bold transition-colors",
                isToday ? "bg-gold text-goldink" : "bg-s2 text-ink2 hover:text-ink"
              )}
              title={isToday ? "Текущая дата" : "Вернуться к сегодня"}
            >
              {dayWord && <span>{dayWord}</span>}
              <span className={cn(dayWord ? "font-medium opacity-80" : "font-semibold")}>
                {dateLabel.replace(/^.,\s*/, "")}
              </span>
            </button>
            <button
            ...
            {/* произвольная дата — иконка календаря в том же кластере */}
            <label
              className="relative flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg bg-s2 text-ink2 transition-colors hover:bg-gold hover:text-goldink"
            ...
            {customDate && (
              <button onClick={() => setCustomDate(null)} className="text-xs font-semibold text-gold hover:text-gold/80" title="Вернуться к сегодня">
                сброс
              </button>
            )}
          </div>
...
function SignalsLegend() {
  const items: { icon: React.ReactNode; text: string }[] = [
    ...
    { icon: <Trophy className="h-3.5 w-3.5 text-gold" />, text: "важный матч: борьба за 1-е место, призовые места или место в таблице" },
