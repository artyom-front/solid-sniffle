...
const SECTIONS: { id: Section; label: string; icon: React.ComponentType<{ className?: string }>; group: string; roles: string[] }[] = [
  ...
  { id: "statblocks", label: "Стат-карточки", icon: BarChart3, group: "Сайт", roles: ["LEAGUE_ADMIN", "SUPER_ADMIN"] },
  { id: "formats", label: "Виды футбола", icon: Shapes, group: "Сайт", roles: ["LEAGUE_ADMIN", "SUPER_ADMIN"] },
...
export default function AdminShell({ user, version, bump, onReload, focusMatchId, onMatchHandled }: Props) {
  ...
  const nav = (onNavigate?: () => void) => (
    <AdminNav groups={groups} sections={visible} section={section} onSelect={(id) => { setSection(id); setSelectedMatchId(null); setFocusTeamId(null); setFocusPersonId(null); onNavigate?.(); }} />
  );
  ...
  return (
    <div className="theme-light flex min-h-screen bg-s0 text-ink">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {NEEDS_SEASON.includes(section) && !activeMatchId && !focusTeamId && !focusPersonId && (
