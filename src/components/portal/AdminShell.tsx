...
type Section =
  ...
  | "protocol" | "kdc" | "schedule" | "registrations" | "merge" | "audit" | "security" | "users";
...
const SECTIONS: { id: Section; label: string; icon: React.ComponentType<{ className?: string }>; group: string; roles: string[] }[] = [
  ...
  { id: "users", label: "Пользователи", icon: Users, group: "Система", roles: ["SUPER_ADMIN"] },
...
export default function AdminShell({ user, version, bump, onReload, focusMatchId, onMatchHandled }: Props) {
  ...
  const renderSection = () => {
    ...
    switch (section) {
      ...
      case "users":
        return <UsersPanel version={version} user={{ id: user.id, email: user.email, role: user.role }} />;
      case "audit":
        return <AuditPanel />;
    }
  };
