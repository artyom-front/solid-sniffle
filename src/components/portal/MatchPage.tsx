...
/** Значок участия игрока в событии: гол/ассист/карточка/замена + минута.
 *  Показывается у завершённых и идущих матчей — сразу видно, кто вышел
 *  на поле и чем отметился. */
function PlayerEventMarks({ events, personId }: { events: EventRow[]; personId: string }) {
  ...
  return (
    <span className="ml-auto flex shrink-0 items-center gap-1">
      {marks.map((mk, i) => (
        <span key={i} title={mk.title} className="flex items-center gap-0.5 rounded bg-s2/70 px-1 py-0.5 font-mono text-[10px] leading-none text-ink2">
          {mk.node}
          <span>{mk.label}</span>
        </span>
      ))}
          ...
          <div key={team.id} className="rounded-xl border border-sline/50 p-3">
            ...
            {starters.map((l) => (
              <button key={l.id} className="flex w-full items-center gap-2.5 rounded-lg px-1 py-1.5 text-left text-ink2 hover:text-ink" onClick={() => navigate(`/player/${l.person.id}`)}>
                ...
                {showMarks && <PlayerEventMarks events={m.events} personId={l.person.id} />}
              </button>
            ))}
            {bench.length > 0 && (
              <>
