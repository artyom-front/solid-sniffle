      ...
      <div className={cn("overflow-hidden rounded-2xl border border-sline bg-s1", signals?.important.flag && "border-gold/40")}>
        <div className="stadium-glow px-4 py-6 sm:px-8">
          <div className="flex items-center justify-center gap-3">
            {/* Хозяева и гости: только герб, название и роль — без лишней фактуры */}
            <TeamHeroColumn teamId={m.homeTeam.id} name={m.homeTeam.name} logoUrl={m.homeTeam.logoUrl} sideLabel="хозяева" />
            ...
            <TeamHeroColumn teamId={m.awayTeam.id} name={m.awayTeam.name} logoUrl={m.awayTeam.logoUrl} sideLabel="гости" />
          </div>
        </div>
...
// Только герб, название и роль в матче. Вся фактура (позиция, серия,
// бомбардир, новый тренер) — во вкладке «Превью»: единая точка знаний.
function TeamHeroColumn({ teamId, name, logoUrl, sideLabel }: {
  teamId: string; name: string; logoUrl?: string | null; sideLabel: string;
}) {
  return (
    <button className="flex min-w-0 flex-1 flex-col items-center gap-2 text-center" onClick={() => navigate(`/team/${teamId}`)}>
      ...
      <span className="min-w-0">
        ...
        <span className="text-xs text-ink3">{sideLabel}</span>
      </span>
    </button>
  );
}
...
function PreviewTab({ m, standings, h2h, missing, signals, insights, user, refresh, onRated }: {
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {/* ---------- Судья и бригада, оценка судейства ---------- */}
      {(m.referee || (m.officials?.length ?? 0) > 0) && <RefereeBlock m={m} referee={m.referee} user={user} avgRating={avgRating} refresh={refresh} onRated={onRated} />}
    </div>
  );
}

/** Блок судейской бригады: главный судья + официальные лица + оценка */
      ...
      <div className="space-y-2 rounded-xl border border-sline/50 p-3">
        {referee ? (
          ...
          >
            ...
            {avgRating !== null && <span className="shrink-0 font-mono text-sm font-bold text-gold">★ {avgRating.toFixed(1)}</span>}
        ...
        <div className="mt-1">
          ...
          {user && referee && user.personId === referee.id && <p className="mt-2 text-xs text-ink3">Судья не может оценивать сам себя</p>}
        </div>
      </div>
    </section>
  );
