...
// ЛЭЙАУТ v1.0.28 — три колонки (как cybersport.ru):
//   • ЦЕНТР — контент ≤800px, никогда не растягивается;
//   • СЛЕВА — актуальные данные («Прямо сейчас»/«Матч тура»),
//     статистика (топ игроков) и реклама (LEFT_TOP/LEFT_BOTTOM);
//   • СПРАВА — лиги и реклама (RIGHT_TOP/RIGHT_BOTTOM).
...
// появление/исчезновение рекламы в боковых колонках НЕ двигает
// центр; виджеты левой колонки рендерятся в каркасах постоянной
// высоты (скелетоны) до прихода данных. Данные overview/banners
// приходят из SSR (server layout) — баннеры в HTML сразу.
...
export default function SiteShell({ overview, banners, children }: Props) {
  ...
  // правая колонка: реклама под лигами
  ...
  return (
    <div className={cn("theme-dark flex min-h-screen flex-col text-ink", !bgBanner && "bg-s0")}>
      ...
      {/* ---------- Три колонки: слева данные/реклама/статистика, центр ≤800px,
          справа лиги/реклама (cybersport-лэйаут). Грид-шаблон фиксированной
          ширины → реклама в колонках не двигает центр (анти-CLS). ---------- */}
      <main className="relative z-10 flex-1">
        <div className="mx-auto grid w-full grid-cols-1 justify-center gap-4 px-4 py-4 min-[1424px]:grid-cols-[300px_minmax(0,800px)_260px]">
          {/* Левая колонка: актуальные данные, статистика, реклама */}
          <aside className="sticky top-[121px] hidden max-h-[calc(100vh-129px)] self-start overflow-y-auto pr-1 scrollbar-s21 min-[1424px]:block">
            <RightRail overview={overview} banners={banners} version={version} layout="rail" side="left" />
          </aside>
          ...
          <div className={cn("mx-auto w-full bg-s0", SITE_WIDTH)}>
            ...
            {/* Главная (узкие экраны): витрина виджетов под лентой;
                на широких — она уже левой колонкой, дубликат прячем */}
            {isHome && (
              <div className="min-[1424px]:hidden">
                <RightRail overview={overview} banners={banners} version={version} layout="grid" />
              </div>
            )}
          ...
          <aside className="sticky top-[121px] hidden max-h-[calc(100vh-129px)] self-start overflow-y-auto pl-1 scrollbar-s21 min-[1424px]:block">
            ...
            <div className="mt-4 space-y-4">
              <BannerSlot banner={rightTopBanner} />
              <BannerSlot banner={rightBottomBanner} />
            </div>
          </aside>
        </div>
      </main>
