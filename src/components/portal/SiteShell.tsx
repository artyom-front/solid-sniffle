...
export default function SiteShell({ overview, banners, children }: Props) {
  ...
  return (
    <div className="theme-dark flex min-h-screen flex-col bg-s0 text-ink">
      ...
      <main className="flex-1">
        <div className="mx-auto grid w-full max-w-[1440px] grid-cols-1 gap-4 px-4 py-4 lg:grid-cols-[260px_minmax(0,1fr)] xl:grid-cols-[260px_minmax(0,1fr)_320px]">
          ...
          <div className="min-w-0">
            {/* Баннер TOP — в центральной колонке: ровно по ширине ленты матчей.
                Слот пуст (не выбран в админке) — блока на клиенте нет вовсе. */}
            {topBanner && (
              <div className="mb-4">
                <a
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold">{topBanner.title}</p>
                    {topBanner.text && <p className="truncate text-xs text-ink2">{topBanner.text}</p>}
                  </div>
                  <span className="shrink-0 rounded-md bg-gold px-2.5 py-1 text-xs font-bold text-goldink">Реклама</span>
                </a>
              </div>
            )}
            {children}
      ...
      {/* ---------- Футер: только копирайт бренда ---------- */}
      <footer className="mt-auto border-t border-sline bg-[#07090d]">
        <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between px-4 py-5 text-xs text-ink3">
          <p>
            <span className="font-black tracking-tight text-ink">{BRAND.name}</span>
            <span className="text-gold"> · </span>
            © {new Date().getFullYear()}
          </p>
          <p className="hidden sm:block">Все данные носят информационный характер</p>
        </div>
      </footer>
    </div>
  );
