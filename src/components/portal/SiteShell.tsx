...
export default function SiteShell({ overview, banners, children }: Props) {
  ...
  return (
    <div className="theme-dark flex min-h-screen flex-col bg-s0 text-ink">
      ...
      {/* Верхний баннер (слот TOP) — ВНУТРИ центральной колонки: ширина
          совпадает с сеткой данных, нет горизонтального «вылезания».
          Слот без активного баннера не рендерится вовсе — без CLS. */}
            ...
            {topBanner && (
              ...
              >
                {topBanner.imageUrl ? (
                  <>
                    { }
                    <img src={topBanner.imageUrl} alt={topBanner.title} className="absolute inset-0 h-full w-full object-cover opacity-90" />
                    <span className="relative z-10 rounded-md bg-s0/80 px-2.5 py-1 text-xs font-bold text-gold backdrop-blur">Реклама</span>
                  </>
                ) : (
                  <>
                    <div>
      ...
      <footer className="mt-auto border-t border-sline bg-[#07090d]">
        <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-2 px-4 py-6 text-xs text-ink3 sm:flex-row sm:items-center sm:justify-between">
