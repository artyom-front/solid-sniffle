...
const KPI_NAV_ROLES: Record<string, string[]> = {
  protocol: ["REFEREE", "CLUB_ADMIN", "LEAGUE_ADMIN", "SUPER_ADMIN"],
  matches: ["LEAGUE_ADMIN", "SUPER_ADMIN"],
  kdc: ["LEAGUE_ADMIN", "SUPER_ADMIN"],
};
...
export default function AdminDashboard({ data, version, onOpenMatch, onNavigate, role }: Props) {
  ...
  // (журнал — раздел супер-админа; остальным хватает recentAudit из дашборда)
  const { data: audit } = useFetch<{ logs: { id: string; userEmail: string | null; entity: string; action: string; createdAt: string }[] }>(
    role === "SUPER_ADMIN" ? "/api/admin/audit?limit=8" : null,
    version
  );

  const kpis = [
    // v1.0.32 · каждая KPI ведёт к месту, где с ней работают — дашборд
    // утилитарный, а не «красивые цифры»
    { label: "Матчей сегодня", value: k?.matchesToday ?? "—", icon: CalendarDays, color: "text-emerald-600 bg-emerald-50", hint: "открыть протоколы", section: "protocol" },
    { label: "Идут сейчас", value: k?.live ?? "—", icon: Radio, color: "text-red-600 bg-red-50", hint: "открыть протоколы", section: "protocol" },
    { label: "Протоколов к вводу", value: k?.protocolsPending ?? "—", icon: ClipboardList, color: "text-amber-600 bg-amber-50", hint: "открыть протоколы", section: "protocol" },
    { label: "Без судьи ≤48 ч", value: k?.withoutReferee48h ?? "—", icon: Flag, color: "text-orange-600 bg-orange-50", hint: "назначить арбитров", section: "matches" },
    { label: "Активных дисквалов", value: k?.activeSuspensions ?? "—", icon: Ban, color: "text-red-600 bg-red-50", hint: "открыть КДК", section: "kdc" },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        {kpis.map((kpi) => {
          const allowed = !!kpi.section && (KPI_NAV_ROLES[kpi.section]?.includes(role) ?? false);
          return (
            <button
              key={kpi.label}
              onClick={() => allowed && onNavigate(kpi.section)}
              className={cn(
                "rounded-xl border border-zinc-200 bg-white p-4 text-left transition-colors",
                allowed ? "hover:border-emerald-300 hover:bg-emerald-50/40" : "cursor-default"
              )}
            >
              ...
              <p className={cn("text-xs", allowed ? "text-emerald-600" : "text-zinc-400")}>{allowed ? `${kpi.hint} →` : kpi.hint}</p>
            </button>
          );
        })}
      </div>
