...
/** v1.0.32 · Смена собственного пароля: требует текущий пароль (+код 2FA,
 *  если включена) — золотой стандарт, украденная сессия не даёт перехватить
 *  аккаунт. Все ДРУГИЕ сессии пользователя при этом закрываются. */
function PasswordChangeCard({ totpEnabled }: { totpEnabled: boolean }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const res = await apiPost("/api/auth/password", { currentPassword: current, newPassword: next, totp: totp || undefined });
    setBusy(false);
    if (!res.ok) return toast.error(res.error, { duration: 7000 });
    setCurrent(""); setNext(""); setRepeat(""); setTotp("");
    toast.success("Пароль изменён — все другие входы этого аккаунта закрыты");
  };
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
      <div className="flex items-center gap-2 border-b border-zinc-100 bg-zinc-50 px-4 py-2.5 text-sm font-bold text-zinc-700">
        <KeyRound className="h-4 w-4 text-emerald-600" /> Смена пароля
      </div>
      <div className="space-y-3 p-4">
        <div className="space-y-1.5">
          <Label className="text-zinc-500">Текущий пароль</Label>
          <Input type="password" value={current} onChange={(e) => setCurrent(e.target.value)} className="border-zinc-200" placeholder="••••••••" />
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label className="text-zinc-500">Новый пароль (мин. 8)</Label>
            <Input type="password" value={next} onChange={(e) => setNext(e.target.value)} className="border-zinc-200" placeholder="••••••••" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-zinc-500">Повторите новый</Label>
            <Input type="password" value={repeat} onChange={(e) => setRepeat(e.target.value)} className="border-zinc-200" placeholder="••••••••" />
          </div>
        </div>
        {totpEnabled && (
          <div className="space-y-1.5">
            <Label className="text-zinc-500">Код 2FA из приложения</Label>
            <Input value={totp} onChange={(e) => setTotp(e.target.value)} maxLength={6} className="border-zinc-200 font-mono" placeholder="123456" />
          </div>
        )}
        <Button size="sm" onClick={submit} disabled={busy || !current || next.length < 8 || next !== repeat} className="bg-emerald-600 text-white hover:bg-emerald-700">
          <KeyRound className="mr-1 h-3.5 w-3.5" /> Сменить пароль
        </Button>
        <p className="text-xs leading-relaxed text-zinc-400">
          Никто (включая супер-администратора) не знает и не видит ваш пароль. Забыли его —
          супер-админ отправит одноразовую ссылку на установку нового (письмом на ваш email
          либо лично, действует 15 минут).
        </p>
      </div>
    </div>
  );
...
export default function SecurityPanel() {
  ...
  const startSetup = async () => {
    if (!setupPassword) {
      toast.error("Введите пароль аккаунта — перегенерация секрета подтверждается паролем");
      return;
    }
    setBusy(true);
    const res = await call({ action: "setup", password: setupPassword });
    setBusy(false);
    if (!res.ok) {
  ...
  const downloadCodes = () => {
    ...
    const text = `SCORESBOX — резервные коды входа\nКаждый код одноразовый. Храните файл в надёжном месте.\n\n${recoveryCodes.join("\n")}\n`;
    const url = URL.createObjectURL(new Blob([text], { type: "text/plain;charset=utf-8" }));
    const a = document.createElement("a");
    a.href = url;
    a.download = "scoresbox-recovery-codes.txt";
    a.click();
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="space-y-4">
        <PasswordChangeCard totpEnabled={!!status?.enabled} />
        {/* ---------- Статус 2FA ---------- */}
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
        <div className="flex items-center gap-2 border-b border-zinc-100 bg-zinc-50 px-4 py-2.5 text-sm font-bold text-zinc-700">
          ...
          {!status?.enabled && stage !== "pairing" && (
            <div className="space-y-2">
              <p className="text-xs leading-snug text-zinc-500">
                Перегенерация секрета подтверждается паролем: она отключает уже
                включённую 2FA, и украденная сессия не сможет тихо ослабить защиту.
              </p>
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input
                  ...
                  aria-label="Пароль для начала настройки 2FA"
                />
                <Button onClick={startSetup} disabled={busy || !setupPassword} className="bg-emerald-600 text-white hover:bg-emerald-700">
              ...
              <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-start">
                ...
                <div className="min-w-0 flex-1 space-y-2 text-xs text-zinc-600">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="break-all rounded bg-white px-2 py-1 font-mono text-sm tracking-wider text-zinc-800">{pairing.secret}</code>
                    <button
