...
import { readFileTextSmart, encodingLabel } from "@/lib/csvEncoding";
...
export function ImportPanel({ bump, version, overview }: { bump: () => void; version: number; overview: OverviewDTO | null }) {
  ...
  const onFile = async (f: File) => {
    // Excel в русской локали сохраняет «CSV» в windows-1251, а не в UTF-8:
    // читаем байты и определяем кодировку сами (UTF-8/UTF-16/cp1251/koi8-r)
    const { text, encoding } = await readFileTextSmart(f);
    setCsv(text);
    setResult(null);
    if (encoding === "windows-1251" || encoding === "koi8-r") {
      toast.success(`Файл загружен: ${f.name} — кириллица восстановлена (${encodingLabel(encoding)})`);
    } else if (encoding === "utf-16le" || encoding === "utf-16be") {
      toast.success(`Файл загружен: ${f.name} — прочитан как ${encodingLabel(encoding)}`);
    } else if (encoding === "unknown") {
      toast.error(`Кодировка не распознана — проверьте текст на «кракозябры». Надёжно: открыть в Excel → Ctrl+A → Ctrl+C → вставить в поле`);
    } else {
