 ...
 *  Экспортирована для юнит-тестов сортировки (45+X до «Перерыва»). */
export function buildRows(events: TimelineEventDTO[], homeTeamId: string, status: string, homeScore?: number | null, awayScore?: number | null): RenderRow[] {
  ...
  for (const e of sorted) {
    ...
    // маркер «Перерыв» — перед ПЕРВЫМ событием второго тайма (минута ≥ 46).
    // События добавленного времени первого тайма (45+X) остаются ВЫШЕ
    // маркера: перерыв наступает после окончания первого тайма целиком.
  ...
  // Завершённый матч: «Перерыв» показываем ВСЕГДА — даже если во втором
  // тайме не было событий (или их нет вовсе): читателю должно быть видно,
  // что первый тайм закончился, а матч завершён. Счёт маркера в этой ветке
  // равен счёту на перерыве: второго тайма в событиях просто нет.
  if (!halftimeInserted && status === "COMPLETED") {
    rows.push({ kind: "marker", label: "Перерыв", score: `${home}:${away}` });
  }
...
export default function MatchTimeline({
  ...
  // «Перерыв 0:0 / Завершён» (пустой протокол ≠ сыгранный тайм).
