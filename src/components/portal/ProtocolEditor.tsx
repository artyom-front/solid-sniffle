...
import { ArrowLeft, Ban, Flag, CheckCircle2, RotateCcw, Trash2, Users, Info, FileText, Loader2, Upload } from "lucide-react";
...
export default function ProtocolEditor({ matchId, user, onBack, bump }: Props) {
  ...
  const [tab, setTab] = useState<"lineup" | "events" | "file" | "finish">("lineup");
    ...
    <div className="space-y-4">
      ...
      <details className="group rounded-xl border border-zinc-200 bg-white">
        ...
        <ol className="list-decimal space-y-1 border-t border-zinc-100 px-8 py-3 pl-10 text-xs text-zinc-500">
          <li><b>Составы</b> — отметьте игроков каждой команды, заявленных на матч (дисквалифицированные помечены красным и заблокированы)</li>
          <li><b>События</b> — голы, карточки, замены по минутам; счёт считается автоматически и становится LIVE</li>
          <li><b>Завершение</b> — нужен назначенный судья; кнопка «Завершить» фиксирует результат, пересчитывает таблицу и включает дисциплинарные баны</li>
          <li>Скан бумажного протокола (фото или PDF) прикрепляется на вкладке «Файл протокола» — для архива и сверки</li>
        </ol>
      </details>
      ...
      <div className="flex gap-1 rounded-lg bg-zinc-100 p-1">
        {[["lineup", "Составы"], ["events", `События (${data.events.length})`], ["file", "Файл протокола"], ["finish", "Завершение"]].map(([id, label]) => (
          <button
            key={id}
            onClick={() => setTab(id as "lineup" | "events" | "file" | "finish")}
            ...
            disabled={id === "finish" || id === "file" ? false : isLocked}
          >
            {label}
