...
import { Plus, Pencil, Trash2, ClipboardPen, CalendarPlus, Flag, X, CheckSquare, Square, AlertTriangle } from "lucide-react";
...
export function MatchesCrudPanel({ bump, version, overview, onOpenProtocol }: { bump: () => void; version: number; overview: OverviewDTO | null; onOpenProtocol: (matchId: string) => void }) {
  ...
  // v1.0.32 · каскадное удаление: явный диалог вместо мёртвого 409
  const [deleteDialog, setDeleteDialog] = useState<(AdminMatch & { isFriendly?: boolean }) | null>(null);
  ...
  const deleteOne = async () => {
    ...
    setDeleteBusy(true);
    const res = await apiPost<{ removed?: { events: number; lineups: number; ratings: number; officials: number } }>(`/api/admin/matches/${deleteDialog.id}`, {}, "DELETE");
    setDeleteBusy(false);
    if (!res.ok) return toast.error(res.error, { duration: 8000 });
    toast.success(`Матч «${deleteDialog.homeTeam.name} — ${deleteDialog.awayTeam.name}» удалён`);
    setDeleteDialog(null);
    setSelected((s) => { const n = new Set(s); n.delete(deleteDialog.id); return n; });
    bump();
  };
  ...
  const bulkDelete = async () => {
    ...
    setBulkBusy(true);
    const res = await apiPost<{ deleted: number; withProtocol: number }>("/api/admin/matches", {
      action: "bulk-delete", ids: [...selected],
    });
    setBulkBusy(false);
    if (!res.ok) return toast.error(res.error);
    setBulkDialog(false);
    toast.success(`Удалено матчей: ${res.data?.deleted ?? 0}${res.data?.withProtocol ? ` (в т.ч. с протоколом: ${res.data.withProtocol})` : ""}`);
    setSelected(new Set());
    bump();
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-zinc-200 bg-white px-3 py-2">
          ...
          {selected.size > 0 && (
            <>
              ...
              <Button size="sm" variant="outline" className="ml-auto h-7 border-red-200 px-2 text-xs text-red-600 hover:bg-red-50" disabled={bulkBusy} onClick={() => setBulkDialog(true)}>
          ...
          return (
            <div key={m.id} className={cn("flex flex-wrap items-center gap-x-4 gap-y-1 border-b border-zinc-50 px-4 py-2.5 last:border-b-0 transition-colors", isSel && "bg-emerald-50/50")}>
              ...
              <div className="ml-auto flex items-center gap-1">
                <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-400 hover:text-red-600" onClick={() => setDeleteDialog(m)} aria-label="Удалить матч">
      ...
      {/* ---------- v1.0.32 · Удаление матча: каскад + предупреждение ---------- */}
      <Dialog open={!!deleteDialog} onOpenChange={(o) => !o && setDeleteDialog(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-700">
              <Trash2 className="h-4 w-4" /> Удалить матч?
            </DialogTitle>
          </DialogHeader>
          {deleteDialog && (
            <div className="space-y-3">
              <p className="text-sm font-semibold text-zinc-800">
                {deleteDialog.homeTeam.name} — {deleteDialog.awayTeam.name}
                {deleteDialog.round ? ` · ${deleteDialog.round} тур` : ""}
                {deleteDialog.homeScore !== null ? ` · счёт ${deleteDialog.homeScore}:${deleteDialog.awayScore}` : ""}
              </p>
              {(deleteDialog.status === "COMPLETED" || deleteDialog.status === "WALKOVER") && (
                <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-xs leading-relaxed text-red-700">
                  <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                  <span>Матч <b>завершён</b>: его результат уйдёт из турнирной таблицы и статистики, авто-дисквалификации с этого матча снимутся. Действие необратимо.</span>
                </div>
              )}
              <div className="space-y-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-xs leading-relaxed">
                <p className="font-semibold text-zinc-700">Удалится навсегда (детали матча):</p>
                <ul className="space-y-1 text-zinc-500">
                  <li>• события протокола — голы, карточки, замены (статистика и таблица пересчитаются)</li>
                  <li>• составы и бригада матча, оценка судейства, файл протокола</li>
                  <li>• авто-дисквалификации, порождённые этим матчем (красная/жёлтые карточки)</li>
                </ul>
                <p className="font-semibold text-zinc-700">Останутся (мастер-данные):</p>
                <ul className="space-y-1 text-zinc-500">
                  <li>• игроки, команды, клубы, стадионы, судьи</li>
                  <li>• заявки на сезон — все люди доступны для других матчей</li>
                </ul>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteDialog(null)}>Не удалять</Button>
            <Button variant="destructive" disabled={deleteBusy} onClick={deleteOne}>
              <Trash2 className="mr-1 h-3.5 w-3.5" /> Удалить безвозвратно
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* ---------- Массовое удаление с каскадом ---------- */}
      <Dialog open={bulkDialog} onOpenChange={setBulkDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-700">
              <Trash2 className="h-4 w-4" /> Удалить выбранные матчи ({selected.size})?
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-zinc-600">
              Удалятся <b>все выбранные матчи</b>, включая сыгранные — вместе с событиями,
              составами, бригадами, оценками и файлами протоколов.
            </p>
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs leading-relaxed text-amber-800">
              <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
              <span>Результаты удаляемых матчей уйдут из таблиц и статистики, дисквалификации пересчитаются. Игроки, команды, судьи и стадионы не затрагиваются. Действие необратимо.</span>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBulkDialog(false)}>Отмена</Button>
            <Button variant="destructive" disabled={bulkBusy} onClick={bulkDelete}>
              <Trash2 className="mr-1 h-3.5 w-3.5" /> Удалить {selected.size} матчей
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
