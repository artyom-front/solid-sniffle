...
interface PlayerDetail {
  player: {
    ...
    coach: {
      ...
      totals: { matches: number; wins: number; draws: number; losses: number; goalsFor: number; goalsAgainst: number };
    };
    registrations: {
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-2xl border border-sline bg-s1">
        <div className="stadium-glow flex flex-wrap items-center gap-4 px-4 py-5 sm:px-6">
          <Avatar name={p.name} id={p.id} photoUrl={p.photoUrl} size="xl" className="ring-2 ring-gold/30" />
      ...
      <div className="overflow-hidden rounded-xl border border-sline bg-s1">
        ...
        {tab === "career" && (
          <div className="p-4">
            ...
            {/* Тренерская карьера — отдельно от игровой и судейской (инвариант раздельной статистики) */}
            {p.coach?.seasons.length > 0 && (
              <div className={cn("rounded-xl border border-sline/50 p-3", (hasReg || isRef) && "mb-4")}>
                <p className="mb-3 flex items-center gap-1.5 text-sm font-bold text-ink"><UserCog className="h-4 w-4 text-gold" /> Тренерская карьера</p>
                <div className="grid grid-cols-3 gap-2 text-center sm:grid-cols-6">
                  {[
                    ["Матчи", p.coach.totals.matches],
                    ["Победы", p.coach.totals.wins],
                    ["Ничьи", p.coach.totals.draws],
                    ["Поражения", p.coach.totals.losses],
                    ["Мячи", `${p.coach.totals.goalsFor}:${p.coach.totals.goalsAgainst}`],
                    ["% побед", p.coach.totals.matches ? Math.round((p.coach.totals.wins / p.coach.totals.matches) * 100) + "%" : "—"],
                  ].map(([label, value]) => (
                    <StatTile key={label as string} value={value as string} label={label as string} accent={label === "Победы"} />
                  ))}
                </div>
                <div className="mt-3 overflow-x-auto scrollbar-s21">
                  <table className="w-full min-w-[460px] text-sm">
                    <thead>
                      <tr className="border-b border-sline/60 text-xs uppercase tracking-wide text-ink3">
                        <th className="px-2 py-2 text-left font-semibold">Сезон</th>
                        <th className="px-2 py-2 text-left font-semibold">Команда</th>
                        <th className="px-2 py-2 text-center font-semibold">Матчи</th>
                        <th className="px-2 py-2 text-center font-semibold">В-Н-П</th>
                        <th className="px-2 py-2 text-center font-semibold">Мячи</th>
                        <th className="px-2 py-2 text-center font-semibold">Очки</th>
                      </tr>
                    </thead>
                    <tbody>
                      {p.coach.seasons.map((s, i) => (
                        <tr key={i} className="border-b border-sline/40 last:border-b-0">
                          <td className="px-2 py-2 text-ink2">{s.season.league} · {s.season.name}</td>
                          <td className="px-2 py-2">
                            <button className="font-medium text-ink hover:text-gold" onClick={() => navigate(`/team/${s.team.id}`)}>{s.team.name}</button>
                          </td>
                          <td className="px-2 py-2 text-center tabular text-ink">{s.matches}</td>
                          <td className="px-2 py-2 text-center tabular text-ink2">{s.wins}-{s.draws}-{s.losses}</td>
                          <td className="px-2 py-2 text-center tabular text-ink2">{s.goalsFor}:{s.goalsAgainst}</td>
                          <td className="px-2 py-2 text-center tabular font-bold text-gold">{s.points}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
