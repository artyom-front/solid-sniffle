...
interface Props {
  ...
  /** Дополнительная подсказка в диалоге блокировки (например, путь через Merge) */
  blockedHint?: React.ReactNode;
}

export function SmartDelete({ endpoint, entityLabel, title = "Удаление", onDone, onOpenDetail, labelOpenDetail = "Открыть карточку", blockedHint }: Props) {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {blocked && (
            <div className="space-y-3">
              ...
              {blockedHint && (
                <div className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2.5 text-xs leading-relaxed text-violet-800">
                  {blockedHint}
                </div>
              )}
