...
export interface BannerDTO {
  ...
  linkUrl: string | null;
  text: string | null;
  /** масштаб картинки: cover | contain | repeat (null — по умолчанию слота) */
  imageFit?: string | null;
  /** позиция: center | top | bottom */
  imagePos?: string | null;
  /** маркировка «Реклама»: шрифт px и прозрачность % */
  markSize?: number | null;
  markOpacity?: number | null;
}

/** Вид футбола в меню сайта (v1.0.29): управляется из админки.
 *  code совпадает с League.format — по нему фильтруется лента. */
export interface FormatLinkDTO {
  code: string;
  label: string;
  sortOrder: number;
}

/** Стат-карточка правой колонки (v1.0.29): редакционный блок статистики
 *  с опциональным фото. Карточка фиксированной высоты — анти-CLS. */
...
export interface OverviewDTO {
  /** виды футбола для меню (видимые, по порядку админа) */
  formats?: FormatLinkDTO[];
  leagues: LeagueDTO[];
  stats: {
...
export interface MatchDTO {
  ...
  homeTeam: { id: string; name: string; clubName: string | null; city?: string | null; logoUrl?: string | null };
  awayTeam: { id: string; name: string; clubName: string | null; city?: string | null; logoUrl?: string | null };
...
export interface PlayerStatRowDTO {
  ...
  /** фото игрока (Media) — для аватара в топе; слот всегда 24×24 (анти-CLS) */
...
export { FORMAT_LABELS, DEFAULT_FORMAT_LINKS, STARTER_LIMITS, REGISTRATION_ROLE_LABELS, POSITION_LABELS, ROLE_LABELS, EVENT_LABELS, SOURCE_LABELS, STATUS_LABELS } from "@/lib/labels";
