...
async function main() {
  ...
  // v1.0.32: liga2@ — ДЕМО скоуп-админа конкретной лиги (Первая лига):
  // проверяйте ограничение доступа прямо в панели. Боевые пользователи
  // создаются БЕЗ пароля и получают одноразовую ссылку установки.
  const firstLeague = await db.league.findFirst({ orderBy: { createdAt: "asc" } });
  const [superAdmin, leagueAdmin, scopedLeagueAdmin, refereeUser, clubUser] = await Promise.all([
    // seed-юзеры входят по паролю сразу: emailVerified/passwordSet = true
    // (v1.0.33; боевые создаются без пароля и приглашаются письмом/ссылкой)
    db.user.create({ data: { email: "admin@ff21.ru", passwordHash: hashPassword("admin123"), role: "SUPER_ADMIN", emailVerified: true, passwordSet: true } }),
    db.user.create({ data: { email: "liga@ff21.ru", passwordHash: hashPassword("liga123"), role: "LEAGUE_ADMIN", emailVerified: true, passwordSet: true } }),
    db.user.create({
      data: firstLeague
        ? { email: "liga2@ff21.ru", passwordHash: hashPassword("liga2123"), role: "LEAGUE_ADMIN", leagueId: firstLeague.id, emailVerified: true, passwordSet: true }
        : { email: "liga2@ff21.ru", passwordHash: hashPassword("liga2123"), role: "LEAGUE_ADMIN", emailVerified: true, passwordSet: true },
    }),
    db.user.create({ data: { email: "sudya@ff21.ru", passwordHash: hashPassword("sudya123"), role: "REFEREE", personId: referees[0].id, emailVerified: true, passwordSet: true } }),
    db.user.create({ data: { email: "club@ff21.ru", passwordHash: hashPassword("club123"), role: "CLUB_ADMIN", clubId: (await db.club.findFirst({ where: { name: { contains: "Урняк" } } }))!.id, emailVerified: true, passwordSet: true } }),
  ]);
  console.log(`   admin@ / liga@ / liga2@ (скоуп 1-й лиги) / sudya@ / club@ff21.ru · seed ${today}`);
    ...
    data: [
      {
        ...
        text: "Футбольная форма, бутсы и вратарские перчатки со скидкой 15% по промокоду SCORESBOX",
