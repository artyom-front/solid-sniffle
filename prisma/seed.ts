...
async function main() {
  ...
  const [superAdmin, leagueAdmin, scopedLeagueAdmin, refereeUser, clubUser] = await Promise.all([
    ...
    db.user.create({
      data: firstLeague
        ? { email: "liga2@ff21.ru", passwordHash: hashPassword("liga2123"), role: "LEAGUE_ADMIN", leagueId: firstLeague.id }
        : { email: "liga2@ff21.ru", passwordHash: hashPassword("liga2123"), role: "LEAGUE_ADMIN" },
    }),
